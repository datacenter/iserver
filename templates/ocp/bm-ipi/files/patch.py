...
ssh_key_filename = get_file('/home/kni/.ssh/id_rsa.pub')
if ssh_key_filename is None:
    print('Failed to read ssh key file')
    sys.exit(1)
ssh_key_filename = ssh_key_filename.rstrip('\n')

bootstrap_os_image_filename = get_file('/root/bootstrap_os_image')
if bootstrap_os_image_filename is None:
    print('Failed to read bootstrap os image file')
    sys.exit(1)
bootstrap_os_image_filename = bootstrap_os_image_filename.rstrip('\n')
...
config_content = config_content.replace('BOOTSTRAP_OS_IMAGE_FILE', bootstrap_os_image_filename)
