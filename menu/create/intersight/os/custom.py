...
@click.option("--config", "os_name", is_flag=False, show_default=False, default='', type=click.STRING, help="OS Configuration File")
@click.option("--values", "values_filename", is_flag=False, show_default=False, default='', type=click.STRING, help="Values filename")
...
def create_os_install_custom_command(
    ctx,
    iaccount,
    name_filter,
    ip_filter,
    ...
    """OS installation with custom configuration"""

    # iserver create os-install custom
    ...
    try:
        ...
        attributes = os_install_validations.get_custom_attributes(
            ctx,
            iaccount,
            name_filter,
            ip_filter,
