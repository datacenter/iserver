...
@click.pass_obj
@click.option("--ocp", "ocp_cluster_name", is_flag=False, show_default=False, default='', type=click.STRING, help="OCP Cluster Name")
@click.option("--osp", "osp_cluster_name", is_flag=False, show_default=False, default='', type=click.STRING, help="OSP Cluster Name")
...
def create_ocp_migrate_command(
    ...
    """Migrte virtual machine"""

    # iocp create ocp migrate

    try:
        if len(deployment_directory) == 0:
            ctx.my_output.error('Define yaml files location')
            raise ErrorExit

        if not os.path.isdir(deployment_directory):
            ctx.my_output.error('Define yaml files directory location')
            raise ErrorExit
        ...
        osp_handlers = validations.validate_osp_name(ctx, osp_cluster_name)
        if osp_handlers is None:
            raise ErrorExit

        if not osp_ocp_migration.run(ctx, osp_handlers, k8s_handlers, settings, deployment_directory):
            raise ErrorExit
