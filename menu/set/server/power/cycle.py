...
def set_server_power_cycle_command(ctx, iaccount, group, serial_filter, name_filter, ip_filter, no_confirm, no_wait, dry_run, verbose, devel):
    """Power cycle"""

    # iserver set server power cycle
    ...
    try:
        ...
        if powered_off:
            ctx.my_output.default('Some servers are powered off. Power cycle will fail on those servers. Remove powered-off servers from the list?')
        ...
        if not lcm_server_power_handler.power_cycle(servers):
            raise ErrorExit
