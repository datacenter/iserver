...
def set_ocp_node_restart(
    ...
    try:
        ...
        if not ocp_handler.set_ocp_node_down(node_name, wait=True):
            raise ErrorExit

        if not ocp_handler.set_ocp_node_up(node_name, wait=True):
            raise ErrorExit
