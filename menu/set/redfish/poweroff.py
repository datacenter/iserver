...
@click.option("--hard", is_flag=True, show_default=True, default=False, help="Hard (non-graceful)")
@click.option("--timeout", "get_timeout", is_flag=False, show_default=True, default=10, type=click.INT, help="Get uri timeout")
def set_redfish_power_off_command(
        ctx,
        endpoint_type,
        endpoint_ip,
        endpoint_port,
    ...
    try:
        ...
        success = redfish_handler.endpoint_handler.power_off(gracefull=not hard)
        if not success:
            ctx.my_output.error('Server power off failed...')
            raise ErrorExit

        ctx.my_output.default('Power off requested via Redfish API')
