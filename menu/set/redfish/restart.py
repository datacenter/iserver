...
def set_redfish_restart_command(
        ctx,
        endpoint_type,
        endpoint_ip,
        endpoint_port,
    ...
    try:
        ...
        success = redfish_handler.endpoint_handler.power_restart(gracefull=not hard)
        if not success:
            ctx.my_output.error('Server restart failed...')
            raise ErrorExit

        ctx.my_output.default('Restart requested via Redfish API')
