...
def get_k8s_ms_command(
    ...
    try:
        ...
        if output == 'mo':
            migrations = k8s_handlers.get_migration(
        ...
        migrations = k8s_handlers.get_migrations(
            object_filter=object_filter
        )
        ...
        ctx.my_output.json_output(migrations)

        if 'state' in view:
            k8s_output_handler.print_migrations(
                migrations
            )
