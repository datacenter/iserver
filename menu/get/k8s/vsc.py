...
def get_k8s_vsc_command(
    ...
    """Get k8s volume snapshot content"""
    ...
    try:
        ...
        if output == 'mo':
            volume_snapshot_contents = k8s_handlers.get_volume_snapshot_contents(
        ...
        if 'state' in view:
            k8s_output_handler.print_volume_snapshot_contents(
                volume_snapshot_contents,
                title=True
            )
