...
def get_k8s_vmp_command(
    ...
    """Get k8s virtual machine preference"""
    ...
    try:
        ...
        if output == 'mo':
            virtual_machine_prefs = k8s_handlers.get_virtual_machine_prefs(
                object_filter=object_filter,
                return_mo=True
            )
            ctx.my_output.default(
                json.dumps(
                    virtual_machine_prefs,
                    indent=4
                )
            )
            return
        ...
        if 'state' in view:
            k8s_output_handler.print_virtual_machine_prefs(
                virtual_machine_prefs,
                title=True
            )
