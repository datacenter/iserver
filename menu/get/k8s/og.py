...
@click.option("--view", "-v", default=['state'], help="[state|ns|all]", show_default=True, multiple=True)
...
def get_k8s_og_command(
    ...
    try:
        ...
        if output == 'mo':
            operator_groups = k8s_handlers.get_operator_groups(
        ...
        if 'state' in view:
            k8s_output_handler.print_operator_groups(
                operator_groups,
                title=True
            )

        if 'ns' in view:
            k8s_output_handler.print_operator_groups_ns(
                operator_groups,
                title=True
            )
