...
def get_k8s_sts_command(
    ...
    try:
        ...
        if output == 'mo':
            stateful_sets = k8s_handlers.get_stateful_sets(
        ...
        if 'state' in view:
            k8s_output_handler.print_stateful_sets(
                stateful_sets,
                title=True
            )

        if 'metadata' in view:
            k8s_output_handler.print_stateful_sets_metadata(
                stateful_sets,
                title=True
            )
