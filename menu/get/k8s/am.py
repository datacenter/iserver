...
def get_k8s_am_command(
    ...
    try:
        ...
        if output == 'mo':
            alert_managers = k8s_handlers.get_alert_managers(
        ...
        ctx.my_output.json_output(alert_managers)

        if 'state' in view:
            k8s_output_handler.print_alert_managers(
                alert_managers,
                title=True
            )
