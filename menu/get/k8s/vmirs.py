...
def get_k8s_vmirs_command(
    ...
    """Get k8s virtual machine instance replica set"""

    # iserver get k8s vmirs
    ...
    try:
        ...
        if output == 'mo':
            virtual_machine_replica_sets = k8s_handlers.get_virtual_machine_replica_sets(
                object_filter=object_filter,
                return_mo=True
            )
            ctx.my_output.default(
                json.dumps(
                    virtual_machine_replica_sets,
                    indent=4
                )
            )
            return
        ...
        if 'state' in view:
            k8s_output_handler.print_virtual_machine_replica_sets(
                virtual_machine_replica_sets,
                title=True
            )
