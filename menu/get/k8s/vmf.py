...
def get_k8s_vmf_command(
    ...
    """Get k8s virtual machine instance type"""
    ...
    try:
        ...
        if output == 'mo':
            virtual_machine_itypes = k8s_handlers.get_virtual_machine_itypes(
                object_filter=object_filter,
                return_mo=True
            )
            ctx.my_output.default(
                json.dumps(
                    virtual_machine_itypes,
                    indent=4
                )
            )
            return
        ...
        if 'state' in view:
            k8s_output_handler.print_virtual_machine_itypes(
                virtual_machine_itypes,
                title=True
            )
