...
def get_k8s_catsrc_command(
    ...
    """Get k8s catalog sources"""

    # iserver get k8s catsrc
    ...
    try:
        ...
        if output == 'mo':
            catalog_sources = k8s_handlers.get_catalog_sources(
        ...
        ctx.my_output.json_output(catalog_sources)

        if 'state' in view:
            k8s_output_handler.print_catalog_sources(
                catalog_sources,
                title=True
            )
