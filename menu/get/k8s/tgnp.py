...
def get_k8s_tgnp_command(
    ...
    try:
        ...
        if output == 'mo':
            tetragon_network_policies = k8s_handlers.get_tetragon_network_policies(
        ...
        ctx.my_output.json_output(tetragon_network_policies)

        if 'state' in view:
            k8s_output_handler.print_tetragon_network_policies(
                tetragon_network_policies,
                title=True
            )
