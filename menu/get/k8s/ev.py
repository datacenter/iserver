...
@click.option("--kind", default='', callback=validations.empty_string_to_none, help="Filter by kind")
...
@click.option("--message", default='', callback=validations.empty_string_to_none, help="Filter by message")
...
def get_k8s_ev_command(
    ...
    try:
        ...
        if output == 'mo':
            events = k8s_handlers.get_events(
        ...
        if 'state' in view:
            k8s_output_handler.print_events(
                events,
                title=True
            )

        ctx.my_output.default('Filter: namespace, name, kind, message', before_newline=True)
