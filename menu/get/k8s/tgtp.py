...
def get_k8s_tgtp_command(
    ...
    try:
        ...
        if output == 'mo':
            tracing_policies = k8s_handlers.get_tracing_policies(
        ...
        ctx.my_output.json_output(tracing_policies)

        if 'state' in view:
            k8s_output_handler.print_tracing_policies(
                tracing_policies,
                title=True
            )
