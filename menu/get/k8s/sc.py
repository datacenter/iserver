...
@click.option("--view", "-v", default=['state'], help="[state|res]", show_default=True, multiple=True)
...
def get_k8s_sc_command(
    ...
    try:
        ...
        if output == 'mo':
            storage_classes = k8s_handlers.get_storage_classes(
        ...
        ctx.my_output.json_output(storage_classes)

        if 'state' in view:
            k8s_output_handler.print_storage_classes(storage_classes)
        ...
        ctx.my_output.default('View:   state (def), res')
