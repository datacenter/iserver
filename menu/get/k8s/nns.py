...
@click.option("--view", "-v", default=['state'], help="[state|dns|route|bond|eth|lb|lldp|ovs|vf|vlan|intf|route|ethtool|all]", show_default=True, multiple=True)
...
def get_k8s_nns_command(
    ...
    try:
        success = common.get(
            ...
            output,
            view,
            'state (def), dns, route, intf, bond, eth, vlan, vf, lb, ovs, lldp, ethtool, all',
