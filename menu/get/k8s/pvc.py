...
@click.option("--view", "-v", default=['state'], help="[state|metadata|usage|all]", show_default=True, multiple=True)
...
def get_k8s_pvc_command(
    ...
    """Get k8s persistent volume claim"""
    ...
    try:
        ...
        if 'state' in view:
            k8s_output_handler.print_pvcs_base(pvcs)

        if 'usage' in view:
            k8s_output_handler.print_pvcs(pvcs)

        if 'metadata' in view:
            k8s_output_handler.print_pvcs_metadata(pvcs)

        ctx.my_output.default('Filter: namespace, name', before_newline=True)
        ctx.my_output.default('View:   state (def), metadata, usage, all')
