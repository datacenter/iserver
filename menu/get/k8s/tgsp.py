...
def get_k8s_tgsp_command(
    ...
    try:
        ...
        if output == 'mo':
            sandbox_policies = k8s_handlers.get_sandbox_policies(
        ...
        ctx.my_output.json_output(sandbox_policies)

        if 'state' in view:
            k8s_output_handler.print_sandbox_policies(
                sandbox_policies,
                title=True
            )
