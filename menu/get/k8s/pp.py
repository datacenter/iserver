...
@click.option("--name", default='', callback=validations.empty_string_to_none, help="Filter by performance profile name")
...
def get_k8s_pp_command(
    ...
    """Get k8s performance profile (ocp)"""
    ...
    try:
        ...
        if output == 'mo':
            performance_profiles = k8s_handlers.get_performance_profiles(
        ...
        if len(performance_profiles) == 0:
            ctx.my_output.error(
                'No performance profile found matching the search criteria'
            )
            raise ErrorExit

        if 'state' in view:
            k8s_output_handler.print_performance_profiles(
                performance_profiles,
                title=True
            )
