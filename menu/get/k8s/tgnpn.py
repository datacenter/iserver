...
def get_k8s_tgnpn_command(
    ...
    """Get k8s tetragon network policy"""
    ...
    try:
        ...
        if output == 'mo':
            tetragon_network_policies_namespaced = k8s_handlers.get_tetragon_network_policies_namespaced(
        ...
        ctx.my_output.json_output(tetragon_network_policies_namespaced)

        if 'state' in view:
            k8s_output_handler.print_tetragon_network_policies_namespaced(
                tetragon_network_policies_namespaced,
                title=True
            )
