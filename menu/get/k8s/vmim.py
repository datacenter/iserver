...
def get_k8s_vmim_command(
    ...
    """Get k8s virtual machine instance migration"""

    # iserver get k8s vmim
    ...
    try:
        ...
        if output == 'mo':
            virtual_machine_migrations = k8s_handlers.get_virtual_machine_migrations(
                object_filter=object_filter,
                return_mo=True
            )
            ctx.my_output.default(
                json.dumps(
                    virtual_machine_migrations,
                    indent=4
                )
            )
            return
        ...
        if 'state' in view:
            k8s_output_handler.print_virtual_machine_migrations(
                virtual_machine_migrations,
                title=True
            )
