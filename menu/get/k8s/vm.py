...
@click.option("--view", "-v", default=['state'], help="[state|metadata|dv|all]", show_default=True, multiple=True)
...
def get_k8s_vm_command(
    ...
    try:
        ...
        if output == 'mo':
            virtual_machines = k8s_handlers.get_virtual_machines(
        ...
        ctx.my_output.json_output(virtual_machines)

        if 'state' in view:
            k8s_output_handler.print_virtual_machines(
                virtual_machines
            )

        if 'metadata' in view:
            k8s_output_handler.print_virtual_machines_metadata(
                virtual_machines,
                title=True
            )

        if 'dv' in view:
            k8s_output_handler.print_virtual_machines_dv(
                virtual_machines,
                title=True
            )

        ctx.my_output.default('Filter: namespace, name', before_newline=True)
        ctx.my_output.default('View:   state (def), metadata, dv, all')
