...
def get_k8s_bmh_command(
    ...
    try:
        ...
        if output == 'mo':
            bare_metal_hosts = k8s_handlers.get_bare_metal_hosts(
        ...
        ctx.my_output.json_output(bare_metal_hosts)

        if 'state' in view:
            k8s_output_handler.print_bare_metal_hosts(
                bare_metal_hosts,
                title=True
            )
