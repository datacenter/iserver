...
def get_k8s_lv_command(
    ...
    try:
        ...
        if output == 'mo':
            lvs = k8s_handlers.get_logical_volumes(
                return_mo=True
            )
        ...
        ctx.my_output.json_output(lvs)

        if 'state' in view:
            k8s_output_handler.print_logical_volumes(
                lvs
            )
