...
@click.option("--view", "-v", default=['state'], help="[state|dep|all]", show_default=True, multiple=True)
...
def get_k8s_co_command(
    ...
    try:
        ...
        if output == 'mo':
            cluster_operators = k8s_handlers.get_cluster_operators(
        ...
        ctx.my_output.json_output(cluster_operators)

        if 'state' in view:
            k8s_output_handler.print_cluster_operators(cluster_operators)

        if 'dep' in view:
            k8s_output_handler.print_cluster_operators_deployment(
                cluster_operators,
                title=True
            )

        ctx.my_output.default('Filter: name', before_newline=True)
        ctx.my_output.default('View:   state (def), dep')
