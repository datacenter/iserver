...
def get_k8s_rb_command(
    ...
    try:
        ...
        if output == 'mo':
            crbs = k8s_handlers.get_role_bindings(
        ...
        if 'state' in view:
            k8s_output_handler.print_role_bindings(
                crbs,
                title=True
            )

        ctx.my_output.default('Filter: namespace, name, subject', before_newline=True)
