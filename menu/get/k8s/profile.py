...
@click.option("--view", "-v", default=['state'], help="[state|boot|error|all]", show_default=True, multiple=True)
...
def get_k8s_profile_command(
    ...
    try:
        ...
        if output == 'mo':
            profiles = k8s_handlers.get_profiles(
        ...
        if 'state' in view:
            k8s_output_handler.print_profiles(
                profiles,
                title=True
            )

        if 'boot' in view:
            k8s_output_handler.print_profiles_boot(
                profiles,
                title=True
            )

        if 'error' in view:
            k8s_output_handler.print_profiles_error(
                profiles,
                title=True
            )
