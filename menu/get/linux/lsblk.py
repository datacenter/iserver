...
def get_linux_lsblk_command(
    ...
    try:
        ...
        for name in linux_handlers:
            lsblks[name] = linux_handlers[name].get_lsblks(include_disk_paths=True)
        ...
        ctx.my_output.json_output(lsblks)

        for name in lsblks:
            if lsblks[name] is not None:
