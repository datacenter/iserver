import sys
import json
import threading
import traceback
import click

from lib.linux import output as linux_output

from menu import validations
from menu import progress


class Failure(Exception):
    pass


class ErrorExit(Exception):
    pass


class NoResultExit(Exception):
    pass


@click.command("lvm")
@click.pass_obj
@click.option("--server", multiple=True, help="Linux server name")
@click.option("--no-cache", "no_cache", is_flag=True, show_default=True, default=False, help="Disable cache")
@click.option("--output", "-o", type=click.Choice(['default', 'json'], case_sensitive=False), default='default', show_default=True)
@click.option("--devel", is_flag=True, show_default=True, default=False, help="Developer output")
def get_linux_lvm_command(
        ctx,
        server,
        no_cache,
        output,
        devel
        ):
    """Get lvm"""

    ctx.developer = devel
    ctx.output = output

    try:
        linux_output_handler = linux_output.LinuxOutput(log_id=ctx.run_id)
        linux_handlers = validations.get_linux_handlers(
            ctx,
            server,
            no_cache=no_cache
        )
        if linux_handlers is None:
            raise ErrorExit

        if output not in ['json']:
            ctx.busy = True
            threading.Thread(target=progress.spinner_task, args=(ctx, False,)).start()

        blks = {}
        lvs = {}
        vgs = {}
        pvs = {}

        for name in linux_handlers:
            blks[name] = linux_handlers[name].get_lsblks(include_disk_paths=True)
            lvs[name] = linux_handlers[name].get_lvs(include_pvc=True)
            vgs[name] = linux_handlers[name].get_vgs()
            pvs[name] = linux_handlers[name].get_pvs()

        ctx.busy = False

        if output == 'json':
            ctx.log_prompt = False
            ctx.my_output.default(
                json.dumps(
                    dict(blks=blks, pvs=pvs, vgs=vgs, lvs=lvs),
                    indent=4
                )
            )
            return

        ctx.my_output.json_output(dict(blks=blks, pvs=pvs, vgs=vgs, lvs=lvs))

        for name in blks:
            linux_output_handler.print_linux_lsblk(
                blks[name],
                title=True,
                server=name
            )

            linux_output_handler.print_linux_pv(
                pvs[name],
                title=True,
                server=name
            )

            linux_output_handler.print_linux_vg(
                vgs[name],
                title=True,
                server=name
            )

            linux_output_handler.print_linux_lv(
                lvs[name],
                title=True,
                server=name
            )

    except NoResultExit:
        ctx.busy = False
        sys.exit(666)

    except ErrorExit:
        ctx.busy = False
        sys.exit(1)

    except BaseException:
        ctx.busy = False
        ctx.my_output.traceback(traceback.format_exc())
        sys.exit(1)
