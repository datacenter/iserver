    ...
    try:
        ...
        if vmedia_id < 0:
            for virtual_media_id in range(10):
                ...
                if virtual_media_details is not None:
                    response.append(virtual_media_details)
        ...
        if output == 'default':
            for item in response:
                ctx.my_output.default('Virtual Media [#%s]' % (item['Id']), before_newline=True, underline=True)
                ctx.my_output.default(
