...
def get_ocp_chrony_command(
        ctx,
        cluster_name,
        output,
        devel
        ):
    """Get ocp chrony state"""

    # iserver get ocp chrony
    ...
    try:
        ...
        info = ocp_handler.get_ocp_chrony_info(
            mcp=True,
            config=True,
            state=True
        )
        ctx.busy = False
        ocp_output_handler.print_ocp_chrony_info(info)
