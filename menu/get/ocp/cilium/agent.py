...
from lib.workflow.ocp_cilium_cni import get_agent as ocp_workflow
...
@click.option("--view", "-v", default=['pod'], help="[pod|logs]", show_default=True, multiple=True)
def get_ocp_cilium_agent_command(ctx, cluster_name, name, view):
    ...
    try:
        ...
        ctx.my_output.default('View (-v): pod (def), logs', before_newline=True)
