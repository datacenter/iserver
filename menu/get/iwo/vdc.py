...
def get_iwo_vdc_command(
    ...
    try:
        ...
        vdc = iwo_handler.get_virtual_data_centers(
            resolve_dependencies=show_dep,
            match_rules=match_rules
        )
        if vdc is None:
            ctx.busy = False
            ctx.my_output.error('Failed to get virtual data centers')
            raise ErrorExit
        ...
        ctx.busy = False
        ctx.my_output.json_output(vdc)

        iwo_handler.print_virtual_data_centers(
            vdc,
            show_dep=show_dep,
