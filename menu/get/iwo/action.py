...
def get_iwo_action_command(
    ...
    try:
        ...
        actions = iwo_handler.get_actions(related_class='all')
        if actions is None:
            ctx.busy = False
            ctx.my_output.error('Failed to get actions')
            raise ErrorExit
        ...
        ctx.busy = False
        ctx.my_output.json_output(actions)

        iwo_handler.print_actions_summary(actions)
