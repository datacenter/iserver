...
def get_iwo_target_command(
    ...
    try:
        ...
        targets = iwo_handler.get_targets()
        if targets is None:
            ctx.busy = False
            ctx.my_output.error('Failed to get targets')
            raise ErrorExit
        ...
        ctx.busy = False
        ctx.my_output.json_output(targets)

        iwo_handler.print_targets(targets)
