...
def get_aci_policy_cdp_command(
    ...
    try:
        ...
        policies = apic_handler.get_policies_interface_cdp(
            policy_filter=policy_filter,
            attachment_info=True
        )

        context_name = apic_handler.set_policy_interface_cdp_context(
            policies
        )
        ...
        if 'default' in view:
            aci_output_handler.print_policies_interface_cdp_summary(
                policies
            )

        if 'usage' in view:
            aci_output_handler.print_policies_interface_cdp_usage(
                policies
            )

        if 'intf' in view:
            aci_output_handler.print_policies_interface_cdp_interfaces(
                policies
            )
