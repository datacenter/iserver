...
def get_aci_policy_copp_command(
    ...
    try:
        ...
        policies = apic_handler.get_policies_interface_copp(
            policy_filter=policy_filter,
            attachment_info=True
        )

        context_name = apic_handler.set_policy_interface_copp_context(
            policies
        )
        ...
        if 'default' in view:
            aci_output_handler.print_policies_interface_copp_summary(
                policies
            )

        if 'usage' in view:
            aci_output_handler.print_policies_interface_copp_usage(
                policies
            )

        if 'intf' in view:
            aci_output_handler.print_policies_interface_copp_interfaces(
                policies
            )
