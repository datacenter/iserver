...
def get_aci_policy_l2_command(
    ...
    try:
        ...
        policies = apic_handler.get_policies_interface_l2(
            policy_filter=policy_filter,
            attachment_info=True
        )

        context_name = apic_handler.set_policy_interface_l2_context(
            policies
        )
        ...
        if 'default' in view:
            aci_output_handler.print_policies_interface_l2_summary(
                policies
            )

        if 'usage' in view:
            aci_output_handler.print_policies_interface_l2_usage(
                policies
            )

        if 'intf' in view:
            aci_output_handler.print_policies_interface_l2_interfaces(
                policies
            )
