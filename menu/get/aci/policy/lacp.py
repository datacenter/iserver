...
def get_aci_policy_port_channel_command(
    ...
    # iserver get aci policy interface port_channel
    ...
    try:
        ...
        policies = apic_handler.get_policies_interface_port_channel(
            policy_filter=policy_filter,
            attachment_info=True
        )

        context_name = apic_handler.set_policy_interface_port_channel_context(
            policies
        )
        ...
        if 'default' in view:
            aci_output_handler.print_policies_interface_port_channel_summary(
                policies
            )

        if 'usage' in view:
            aci_output_handler.print_policies_interface_port_channel_usage(
                policies
            )

        if 'intf' in view:
            aci_output_handler.print_policies_interface_port_channel_interfaces(
                policies
            )
