...
def get_aci_policy_storm_control_command(
    ...
    # iserver get aci policy interface storm_control
    ...
    try:
        ...
        policies = apic_handler.get_policies_interface_storm_control(
            policy_filter=policy_filter,
            attachment_info=True
        )

        context_name = apic_handler.set_policy_interface_storm_control_context(
            policies
        )
        ...
        if 'default' in view:
            aci_output_handler.print_policies_interface_storm_control_summary(
                policies
            )

        if 'usage' in view:
            aci_output_handler.print_policies_interface_storm_control_usage(
                policies
            )

        if 'intf' in view:
            aci_output_handler.print_policies_interface_storm_control_interfaces(
                policies
            )
