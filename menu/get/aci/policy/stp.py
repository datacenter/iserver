...
def get_aci_policy_stp_command(
    ...
    try:
        ...
        policies = apic_handler.get_policies_interface_stp(
            policy_filter=policy_filter,
            attachment_info=True
        )

        context_name = apic_handler.set_policy_interface_stp_context(
            policies
        )
        ...
        if 'default' in view:
            aci_output_handler.print_policies_interface_stp_summary(
                policies
            )

        if 'usage' in view:
            aci_output_handler.print_policies_interface_stp_usage(
                policies
            )

        if 'intf' in view:
            aci_output_handler.print_policies_interface_stp_interfaces(
                policies
            )
