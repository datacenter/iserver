...
def get_aci_policy_port_security_command(
    ...
    # iserver get aci policy interface port_security
    ...
    try:
        ...
        policies = apic_handler.get_policies_interface_port_security(
            policy_filter=policy_filter,
            attachment_info=True
        )

        context_name = apic_handler.set_policy_interface_port_security_context(
            policies
        )
        ...
        if 'default' in view:
            aci_output_handler.print_policies_interface_port_security_summary(
                policies
            )

        if 'usage' in view:
            aci_output_handler.print_policies_interface_port_security_usage(
                policies
            )

        if 'intf' in view:
            aci_output_handler.print_policies_interface_port_security_interfaces(
                policies
            )
