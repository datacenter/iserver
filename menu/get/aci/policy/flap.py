...
def get_aci_policy_flap_command(
    ...
    try:
        ...
        policies = apic_handler.get_policies_interface_link_flap(
            policy_filter=policy_filter,
            attachment_info=True
        )

        context_name = apic_handler.set_policy_interface_link_flap_context(
            policies
        )
        ...
        if 'default' in view:
            aci_output_handler.print_policies_interface_link_flap_summary(
                policies
            )

        if 'usage' in view:
            aci_output_handler.print_policies_interface_link_flap_usage(
                policies
            )

        if 'intf' in view:
            aci_output_handler.print_policies_interface_link_flap_interfaces(
                policies
            )
