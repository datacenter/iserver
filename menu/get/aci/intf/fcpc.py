...
def get_aci_node_intf_fcpc_command(
    ...
    # iserver get aci node intf fcpc
    ...
    try:
        ...
        for apic_handler in apic_handlers:
            for node_info in apic_handler['nodes']:
                node_interfaces = apic_handler['handler'].get_interfaces_fcpc(
                    node_info['podId'],
                    node_info['id'],
        ...
        if 'state' in view:
            aci_output_handler.print_interfaces_fcpc_state(
                interfaces,
                title=True
            )

        if 'fault' in view:
            aci_output_handler.print_interface_fcpc_fault_inst(
                fault_inst,
                title=True
            )

        if 'hfault' in view:
            aci_output_handler.print_interface_fcpc_fault_record(
                fault_record,
                when=fault_when,
                title=True
            )
        ...
        if 'event' in view:
            aci_output_handler.print_interface_fcpc_event_logs(
                event,
                when=fault_when,
                title=True
            )
        ...
        if 'audit' in view:
            aci_output_handler.print_interface_fcpc_audit_logs(
                audit,
                when=fault_when,
                title=True
            )
