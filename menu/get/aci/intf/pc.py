...
@click.option("--view", "-v", default=['state'], help="[state|phy|lacp|vlan|stats|fault|hfault|event|audit|diag|all]", show_default=True, multiple=True)
...
def get_aci_node_intf_pc_command(
    ...
    view = validations.validate_view(
        ctx,
        view,
        'state|phy|lacp|vlan|stats|fault|hfault|event|audit|diag|all',
        'state',
        [
    ...
    try:
        ...
        if 'phy' in view:
            aci_output_handler.print_interfaces_port_channel_member(
                interfaces,
                title=True
            )

        if 'vlan' in view:
            aci_output_handler.print_interfaces_port_channel_vlan(
                interfaces,
                title=True
            )

        if 'stats' in view:
            aci_output_handler.print_interfaces_port_channel_stats(
                interfaces,
                title=True
            )

            aci_output_handler.print_interfaces_port_channel_ether(
                interfaces,
                title=True
            )
        ...
        if 'lacp' in view:
            ...
            for interface_info in interfaces:
                lacp = lacp + interface_info['lacp']

            aci_output_handler.print_interfaces_port_channel_lacp(
                lacp,
                title=True
            )

        if 'fault' in view:
            aci_output_handler.print_interface_port_channel_fault_inst(
                fault_inst,
                title=True
            )

        if 'hfault' in view:
            aci_output_handler.print_interface_port_channel_fault_record(
                fault_record,
                when=fault_when,
                title=True
            )
        ...
        if 'event' in view:
            aci_output_handler.print_interface_port_channel_event_logs(
                event,
                when=fault_when,
                title=True
            )
        ...
        if 'audit' in view:
            aci_output_handler.print_interface_port_channel_audit_logs(
                audit,
                when=fault_when,
                title=True
            )
