...
@click.option("--output", "-o", type=click.Choice(['default', 'trans', 'vlan', 'epg', 'load', 'eee', 'nei', 'cdp', 'lldp', 'pg', 'pol', 'aaep', 'ether', 'err', 'qos', 'live', 'verbose', 'json'], case_sensitive=False), default='default', show_default=True)
...
def get_aci_node_intf_phy_command(
    ...
    try:
        ...
        if output in ['verbose', 'trans']:
            fc_stats_info = True

        if output in ['verbose', 'load']:
            load_info = True

        if output in ['verbose', 'qos', 'live']:
            qos_info = True

        if output in ['verbose', 'ether', 'err']:
            ether_stats_info = True

        if output in ['verbose', 'eee']:
            eee_info = True

        if output in ['verbose', 'epg', 'vlan']:
            epg_stats_info = True

        if output in ['verbose', 'nei']:
            cdp_info = True
            lldp_info = True

        if output in ['verbose', 'cdp']:
            cdp_info = True

        if output in ['verbose', 'lldp']:
            lldp_info = True

        if output in ['verbose', 'pol', 'pg', 'aaep']:
            policy_info = True
