    ...
    try:
        ...
        for apic_handler in apic_handlers:
            for node_info in apic_handler['nodes']:
                node_interfaces = apic_handler['handler'].get_interfaces_vfc(
                    node_info['podId'],
                    node_info['id'],
        ...
        if 'state' in view:
            aci_output_handler.print_interfaces_vfc_state(
                interfaces,
                title=True
            )

        if 'fault' in view:
            aci_output_handler.print_interface_vfc_fault_inst(
                fault_inst,
                title=True
            )

        if 'hfault' in view:
            aci_output_handler.print_interface_vfc_fault_record(
                fault_record,
                when=fault_when,
                title=True
            )
        ...
        if 'event' in view:
            aci_output_handler.print_interface_vfc_event_logs(
                event,
                when=fault_when,
                title=True
            )
        ...
        if 'audit' in view:
            aci_output_handler.print_interface_vfc_audit_logs(
                audit,
                when=fault_when,
                title=True
            )
