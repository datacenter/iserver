...
@click.option("--ip", "ip_address", default='', callback=validations.validate_ip, help="IP Address filter")
@click.option("--subnet", "ip_subnet", default='', callback=validations.validate_ip_subnet, help="IP Subnet filter")
@click.option("--output", "-o", type=click.Choice(['state', 'addr', 'counter', 'verbose', 'json'], case_sensitive=False), default='state', show_default=True)
