...
def get_aci_node_intf_cloudsec_command(
    ...
    try:
        ...
        if 'state' in view:
            aci_output_handler.print_interfaces_cloudsec_state(
                interfaces,
                title=True
            )

        if 'fault' in view:
            aci_output_handler.print_interface_cloudsec_fault_inst(
                fault_inst,
                title=True
            )

        if 'hfault' in view:
            aci_output_handler.print_interface_cloudsec_fault_record(
                fault_record,
                when=fault_when,
                title=True
            )
        ...
        if 'event' in view:
            aci_output_handler.print_interface_cloudsec_event_logs(
                event,
                when=fault_when,
                title=True
            )
        ...
        if 'audit' in view:
            aci_output_handler.print_interface_cloudsec_audit_logs(
                audit,
                when=fault_when,
                title=True
            )
