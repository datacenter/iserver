...
@click.option("--output", "-o", type=click.Choice(['state', 'address', 'verbose', 'json'], case_sensitive=False), default='state', show_default=True)
...
def get_aci_node_intf_vpc_command(
    ...
    try:
        ...
        members_info = True
        if output == 'address' and member == 'any':
            members_info = False
