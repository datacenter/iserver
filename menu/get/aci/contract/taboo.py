...
def get_aci_contract_taboo_command(
        ctx,
        controller,
        controller_ip,
        controller_port,
    ...
    try:
        ...
        contracts = apic_handler.get_taboo_contracts(
        ...
        if 'state' in view:
            aci_output_handler.print_taboo_contracts(
                contracts,
        ...
        if 'usage' in view:
            aci_output_handler.print_taboo_contracts_usage(
                contracts,
                title=True
            )

        if 'fault' in view:
            aci_output_handler.print_taboo_contracts_fault_inst(
                fault_inst,
                title=True
            )

        if 'hfault' in view:
            aci_output_handler.print_taboo_contracts_fault_record(
                fault_record,
                when=fault_when,
                title=True
            )
        ...
        if 'event' in view:
            aci_output_handler.print_taboo_contracts_event_logs(
                event,
                when=fault_when,
                title=True
            )
        ...
        if 'audit' in view:
            aci_output_handler.print_taboo_contracts_audit_logs(
                audit,
                when=fault_when,
                title=True
            )
