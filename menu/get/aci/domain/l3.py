...
def get_aci_domain_l3_command(
    ...
    try:
        ...
        domains = apic_handler.get_domains_l3(
        ...
        if 'state' in view:
            aci_output_handler.print_domains_l3(
                domains,
                title=True
            )

        if 'node' in view:
            aci_output_handler.print_domains_l3_node(
                domains,
                title=True
            )

        if 'intf' in view:
            aci_output_handler.print_domains_l3_interface(
                domains,
                title=True
            )

        if 'vlan' in view:
            aci_output_handler.print_domains_l3_vlan(
                domains,
                title=True
            )

        if 'reln' in view:
            aci_output_handler.print_domains_l3_reln(
                domains,
                title=True
            )

        if 'fault' in view:
            aci_output_handler.print_domains_l3_fault_inst(
                fault_inst,
                title=True
            )

        if 'hfault' in view:
            aci_output_handler.print_domains_l3_fault_record(
                fault_record,
                when=fault_when,
                title=True
            )
        ...
        if 'event' in view:
            aci_output_handler.print_domains_l3_event_logs(
                event,
                when=fault_when,
                title=True
            )
        ...
        if 'audit' in view:
            aci_output_handler.print_domains_l3_audit_logs(
                audit,
                when=fault_when,
                title=True
            )
