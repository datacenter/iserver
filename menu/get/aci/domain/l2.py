...
def get_aci_domain_l2_command(
    ...
    try:
        ...
        if 'node' in view:
            aci_output_handler.print_domains_l2_node(
                domains,
                title=True
            )

        if 'intf' in view:
            aci_output_handler.print_domains_l2_interface(
                domains,
                title=True
            )

        if 'vlan' in view:
            aci_output_handler.print_domains_l2_vlan(
                domains,
                title=True
            )

        if 'reln' in view:
            aci_output_handler.print_domains_l2_reln(
                domains,
                title=True
            )

        if 'fault' in view:
            aci_output_handler.print_domains_l2_fault_inst(
                fault_inst,
                title=True
            )

        if 'hfault' in view:
            aci_output_handler.print_domains_l2_fault_record(
                fault_record,
                when=fault_when,
                title=True
            )
        ...
        if 'event' in view:
            aci_output_handler.print_domains_l2_event_logs(
                event,
                when=fault_when,
                title=True
            )
        ...
        if 'audit' in view:
            aci_output_handler.print_domains_l2_audit_logs(
                audit,
                when=fault_when,
                title=True
            )
