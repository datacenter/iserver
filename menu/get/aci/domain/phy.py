...
@click.option("--view", "-v", default=['state'], help="[state|node|intf|vlan|reln|fault|hfault|event|audit|diag|all]", show_default=True, multiple=True)
@click.option("--output", "-o", type=click.Choice(['default', 'json'], case_sensitive=False), default='default', show_default=True)
@click.option("--ttl", "requested_ttl", default=-1, show_default=True, help="Cache ttl")
@click.option("--devel", is_flag=True, show_default=True, default=False, help="Developer output")
def get_aci_domain_phy_command(
        ctx,
        controller,
        controller_ip,
        controller_port,
    ...
    view = validations.validate_view(
        ctx,
        view,
        'state|node|intf|vlan|reln|fault|hfault|event|audit|diag|all',
        'state',
        [
    ...
    try:
        ...
        if 'state' in view:
            aci_output_handler.print_domains_phy(
                domains,
                title=True
            )

        if 'node' in view:
            aci_output_handler.print_domains_phy_node(
                domains,
                title=True
            )

        if 'intf' in view:
            aci_output_handler.print_domains_phy_interface(
                domains,
                title=True
            )

        if 'vlan' in view:
            aci_output_handler.print_domains_phy_vlan(
                domains,
                title=True
            )

        if 'reln' in view:
            aci_output_handler.print_domains_phy_reln(
                domains,
                title=True
            )

        if 'fault' in view:
            aci_output_handler.print_domains_phy_fault_inst(
                fault_inst,
                title=True
            )

        if 'hfault' in view:
            aci_output_handler.print_domains_phy_fault_record(
                fault_record,
                when=fault_when,
                title=True
            )
        ...
        if 'event' in view:
            aci_output_handler.print_domains_phy_event_logs(
                event,
                when=fault_when,
                title=True
            )
        ...
        if 'audit' in view:
            aci_output_handler.print_domains_phy_audit_logs(
                audit,
                when=fault_when,
                title=True
            )

        ctx.my_output.default('Filter: name, aaep, pool, vlan, fault, severity, when', before_newline=True)
        ctx.my_output.default('View:   state (def), node, intf, vlan, reln, fault, hfault, event, audit, diag, all')
