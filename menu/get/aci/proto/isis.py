...
@click.option("--output", "-o", type=click.Choice(['instance', 'intf', 'lsp', 'neighbor', 'route', 'tree', 'tunnel', 'verbose', 'json'], case_sensitive=False), default='instance', show_default=True)
...
def get_aci_node_proto_isis_command(
    ...
    try:
        ...
        intf = []
        intf_info = False
        if output in ['intf', 'verbose']:
            intf_info = True

        lsp = []
        lsp_info = False
        if output in ['lsp', 'verbose']:
            lsp_info = True

        neighbor = []
        neighbor_info = False
        if output in ['neighbor', 'verbose']:
            neighbor_info = True

        route = []
        route_info = False
        if output in ['route', 'verbose']:
            route_info = True

        tree = []
        tree_info = False
        if output in ['tree', 'verbose']:
            tree_info = True

        tunnels = []
        tunnel_info = False
        if output in ['tunnel', 'verbose']:
            tunnel_info = True
