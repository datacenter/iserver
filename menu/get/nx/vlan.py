...
def get_nx_vlan_command(
    ...
    try:
        ...
        for device_handler in device_handlers:
            device_vlans = device_handler['handler'].get_vlans(
                object_filter=object_filter
            )
            if device_vlans is not None:
                vlans = vlans + device_vlans
        ...
        if 'state' in view:
            nexus_output_handler.print_vlans(
                vlans,
                title=True
            )
