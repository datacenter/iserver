...
def get_imc_cli_ipmi_command(
    ...
    try:
        ...
        for item in endpoints:
            ...
            ipmi = endpoint_handler.get_ipmi()
            if ipmi is None:
                ctx.my_output.error('Failed to get ipmi from: %s' % (item['ip']))
            else:
                response.append(
                    ipmi
                )
        ...
        if 'state' in view:
            if output == 'default':
                imc_output_handler.print_imc_ipmi(response)
