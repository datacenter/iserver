...
def get_imc_cli_version_command(
        ctx,
        endpoint_ip,
        username,
        password,
    ...
    try:
        ...
        for item in endpoints:
            ...
            version = endpoint_handler.get_version()
            if version is None:
                ctx.my_output.error('Failed to get version from: %s' % (item['ip']))
            else:
                response.append(
                    version
                )
        ...
        if 'state' in view:
            if output == 'default':
                imc_output_handler.print_imc_version(response)

            if output == 'compare':
                imc_output_handler.print_imc_version_compare(response)
