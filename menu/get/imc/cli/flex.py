...
def get_imc_cli_flex_command(
    ...
    try:
        ...
        for item in endpoints:
            ...
            flex = endpoint_handler.get_flex()
            if flex is None:
                ctx.my_output.error('Failed to get flex from: %s' % (item['ip']))
            else:
                response.append(
                    flex
                )
        ...
        if 'state' in view:
            if output == 'default':
                imc_output_handler.print_imc_flex(response)

            if output == 'compare':
                imc_output_handler.print_imc_flex_compare(response)
