...
def get_imc_cli_cpu_command(
    ...
    try:
        ...
        for item in endpoints:
            ...
            cpu = endpoint_handler.get_cpu()
            if cpu is None:
                ctx.my_output.error('Failed to get cpu from: %s' % (item['ip']))
            else:
        ...
        if 'state' in view:
            if output == 'default':
                imc_output_handler.print_imc_cpu(response)

            if output == 'compare':
                imc_output_handler.print_imc_cpu_compare(response)
