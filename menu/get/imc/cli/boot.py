...
@click.option("--view", "-v", default=['order'], help="[order|device|all]", show_default=True, multiple=True)
@click.option("--output", "-o", type=click.Choice(['default', 'json'], case_sensitive=False), default='default', show_default=True)
@click.option("--devel", is_flag=True, show_default=True, default=False, help="Developer output")
def get_imc_cli_boot_command(
        ctx,
        endpoint_ip,
        username,
        password,
    ...
    try:
        ...
        for item in endpoints:
            ...
            boot = endpoint_handler.get_boot(
                order_info=order_info,
                device_info=device_info
            )
            if boot is None:
                ctx.my_output.error('Failed to get boot from: %s' % (item['ip']))
            else:
                if order_info:
                    response_order = response_order + boot['order']
                if device_info:
                    response_device = response_device + boot['device']
        ...
        response['device'] = response_device
        ...
        if 'order' in view:
            if output == 'default':
                imc_output_handler.print_imc_boot(response_order)

        if 'device' in view:
            if output == 'default':
                imc_output_handler.print_imc_boot_device(response_device)

        ctx.my_output.default('Filter: --', before_newline=True)
        ctx.my_output.default('View:   order (def), device')
        ctx.my_output.default('Output: default (def), json')
