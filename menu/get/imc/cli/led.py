...
def get_imc_cli_led_command(
    ...
    try:
        ...
        for item in endpoints:
            ...
            led = endpoint_handler.get_led()
            if led is None:
                ctx.my_output.error('Failed to get led from: %s' % (item['ip']))
            else:
                response = response + led
        ...
        if 'state' in view:
            if output == 'default':
                imc_output_handler.print_imc_led(response)
