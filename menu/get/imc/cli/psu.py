...
def get_imc_cli_psu_command(
    ...
    try:
        ...
        for item in endpoints:
            ...
            psu = endpoint_handler.get_psu()
            if psu is None:
                ctx.my_output.error('Failed to get psu from: %s' % (item['ip']))
            else:
                response = response + psu
        ...
        if 'state' in view:
            if output == 'default':
                imc_output_handler.print_imc_psu(response)

            if output == 'compare':
                imc_output_handler.print_imc_psu_compare(response)
