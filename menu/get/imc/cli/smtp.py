...
def get_imc_cli_smtp_command(
    ...
    try:
        ...
        for item in endpoints:
            ...
            smtp = endpoint_handler.get_smtp()
            if smtp is None:
                ctx.my_output.error('Failed to get smtp from: %s' % (item['ip']))
            else:
                response.append(
                    smtp
                )
        ...
        if 'state' in view:
            imc_output_handler.print_imc_smtp(response)
