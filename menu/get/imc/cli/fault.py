...
def get_imc_cli_fault_command(
    ...
    try:
        ...
        for item in endpoints:
            ...
            fault = endpoint_handler.get_fault()
            if fault is None:
                ctx.my_output.error('Failed to get fault from: %s' % (item['ip']))
            else:
                response.append(
                    fault
                )
        ...
        if 'state' in view:
            if output == 'default':
                imc_output_handler.print_imc_fault(response)
