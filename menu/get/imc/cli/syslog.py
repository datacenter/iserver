...
def get_imc_cli_syslog_command(
        ctx,
        endpoint_ip,
        username,
        password,
    ...
    try:
        ...
        for item in endpoints:
            ...
            syslog = endpoint_handler.get_syslog()
            if syslog is None:
                ctx.my_output.error('Failed to get syslog from: %s' % (item['ip']))
            else:
                response.append(
                    syslog
                )
        ...
        if 'state' in view:
            if output == 'default':
                imc_output_handler.print_imc_syslog(response)
