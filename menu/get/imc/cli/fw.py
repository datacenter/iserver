...
def get_imc_cli_fw_command(
    ...
    try:
        ...
        for item in endpoints:
            ...
            fw = endpoint_handler.get_fw()
            if fw is None:
                ctx.my_output.error('Failed to get fw from: %s' % (item['ip']))
            else:
                response.append(
                    fw
                )
        ...
        if 'state' in view:
            if output == 'default':
                imc_output_handler.print_imc_fw(response)

            if output == 'compare':
                imc_output_handler.print_imc_fw_compare(response)
