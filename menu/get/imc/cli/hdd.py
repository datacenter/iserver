...
def get_imc_cli_hdd_command(
    ...
    try:
        ...
        for item in endpoints:
            ...
            hdd = endpoint_handler.get_hdd()
            if hdd is None:
                ctx.my_output.error('Failed to get hdd from: %s' % (item['ip']))
            else:
                response = response + hdd
        ...
        if 'state' in view:
            if output == 'default':
                imc_output_handler.print_imc_hdd(response)
