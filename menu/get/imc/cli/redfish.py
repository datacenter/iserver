...
def get_imc_cli_redfish_command(
        ctx,
        endpoint_ip,
        username,
        password,
    ...
    try:
        ...
        for item in endpoints:
            ...
            redfish = endpoint_handler.get_redfish()
            if redfish is None:
                ctx.my_output.error('Failed to get redfish from: %s' % (item['ip']))
            else:
                response.append(
                    redfish
                )
        ...
        if 'state' in view:
            if output == 'default':
                imc_output_handler.print_imc_redfish(response)
