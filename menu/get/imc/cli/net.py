...
def get_imc_cli_net_command(
    ...
    try:
        ...
        for item in endpoints:
            ...
            net = endpoint_handler.get_net()
            if net is None:
                ctx.my_output.error('Failed to get net from: %s' % (item['ip']))
            else:
                response = response + net
        ...
        if 'state' in view:
            if output == 'default':
                imc_output_handler.print_imc_net(response)
