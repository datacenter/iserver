...
def get_imc_cli_sel_command(
    ...
    try:
        ...
        for item in endpoints:
            ...
            sel = endpoint_handler.get_sel()
            if sel is None:
                ctx.my_output.error('Failed to get sel from: %s' % (item['ip']))
            else:
                response.append(
                    sel
                )
        ...
        if 'state' in view:
            if output == 'default':
                imc_output_handler.print_imc_sel(response)

            if output == 'compare':
                imc_output_handler.print_imc_sel_compare(response)
