...
def get_imc_cli_chassis_command(
        ctx,
        endpoint_ip,
        username,
        password,
    ...
    try:
        ...
        for item in endpoints:
            ...
            chassis = endpoint_handler.get_chassis()
            if chassis is None:
                ctx.my_output.error('Failed to get chassis from: %s' % (item['ip']))
            else:
                response.append(
                    chassis
                )
        ...
        if 'state' in view:
            if output == 'default':
                imc_output_handler.print_imc_chassis(response)

            if output == 'compare':
                imc_output_handler.print_imc_chassis_compare(response)
