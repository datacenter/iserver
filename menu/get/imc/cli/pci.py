...
def get_imc_cli_pci_command(
    ...
    try:
        ...
        for item in endpoints:
            ...
            pci = endpoint_handler.get_pci()
            if pci is None:
                ctx.my_output.error('Failed to get pci from: %s' % (item['ip']))
            else:
                response = response + pci
        ...
        if 'state' in view:
            if output == 'default':
                imc_output_handler.print_imc_pci(response)
