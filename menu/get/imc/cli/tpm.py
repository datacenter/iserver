...
def get_imc_cli_tpm_command(
    ...
    try:
        ...
        for item in endpoints:
            ...
            tpm = endpoint_handler.get_tpm()
            if tpm is None:
                ctx.my_output.error('Failed to get tpm from: %s' % (item['ip']))
            else:
                response.append(
                    tpm
                )
        ...
        if 'state' in view:
            if output == 'default':
                imc_output_handler.print_imc_tpm(response)

            if output == 'compare':
                imc_output_handler.print_imc_tpm_compare(response)
