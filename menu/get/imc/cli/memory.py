...
def get_imc_cli_memory_command(
        ctx,
        endpoint_ip,
        username,
        password,
    ...
    try:
        ...
        for item in endpoints:
            ...
            memory = endpoint_handler.get_memory()
            if memory is None:
                ctx.my_output.error('Failed to get memory from: %s' % (item['ip']))
            else:
                response.append(
                    memory
                )
        ...
        if 'state' in view:
            if output == 'default':
                imc_output_handler.print_imc_memory(response)

            if output == 'compare':
                imc_output_handler.print_imc_memory_compare(response)
