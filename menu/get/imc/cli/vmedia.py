...
def get_imc_cli_vmedia_command(
        ctx,
        endpoint_ip,
        username,
        password,
    ...
    try:
        ...
        for item in endpoints:
            ...
            vmedia = endpoint_handler.get_vmedia()
            if vmedia is None:
                ctx.my_output.error('Failed to get vmedia from: %s' % (item['ip']))
            else:
                response.append(
                    vmedia
                )
        ...
        if 'state' in view:
            if output == 'default':
                imc_output_handler.print_imc_vmedia(response)
