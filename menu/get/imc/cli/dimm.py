...
def get_imc_cli_dimm_command(
    ...
    try:
        ...
        for item in endpoints:
            ...
            dimm = endpoint_handler.get_dimm()
            if dimm is None:
                ctx.my_output.error('Failed to get dimm from: %s' % (item['ip']))
            else:
                response = response + dimm
        ...
        if 'state' in view:
            if output == 'default':
                imc_output_handler.print_imc_dimm(response)
