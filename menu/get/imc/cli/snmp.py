...
@click.option("--view", "-v", default=['state'], help="[state|server|user|all]", show_default=True, multiple=True)
@click.option("--output", "-o", type=click.Choice(['default', 'json'], case_sensitive=False), default='default', show_default=True)
@click.option("--devel", is_flag=True, show_default=True, default=False, help="Developer output")
def get_imc_cli_snmp_command(
        ctx,
        endpoint_ip,
        username,
        password,
    ...
    try:
        ...
        for item in endpoints:
            ...
            snmp = endpoint_handler.get_snmp()
            if snmp is None:
                ctx.my_output.error('Failed to get snmp from: %s' % (item['ip']))
            else:
                response.append(
                    snmp
                )
        ...
        if 'state' in view:
            imc_output_handler.print_imc_snmp(response)

        if 'server' in view:
            imc_output_handler.print_imc_snmp_server(response)

        if 'user' in view:
            imc_output_handler.print_imc_snmp_user(response)

        ctx.my_output.default('Filter: --', before_newline=True)
        ctx.my_output.default('View:   state, server, user, all')
