...
def get_imc_cli_bbu_command(
    ...
    """Get imc battery backup unit of storage controller"""
    ...
    try:
        ...
        for item in endpoints:
            ...
            bbu = endpoint_handler.get_bbu()
            if bbu is None:
                ctx.my_output.error('Failed to get bbu from: %s' % (item['ip']))
            else:
                response = response + bbu
        ...
        if 'state' in view:
            if output == 'default':
                imc_output_handler.print_imc_bbu(
                    response
                )

            if output == 'compare':
                imc_output_handler.print_imc_bbu_compare(
                    response
                )
