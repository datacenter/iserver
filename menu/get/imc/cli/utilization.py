...
def get_imc_cli_utilization_command(
        ctx,
        endpoint_ip,
        username,
        password,
    ...
    try:
        ...
        for item in endpoints:
            ...
            utilization = endpoint_handler.get_utilization()
            if utilization is None:
                ctx.my_output.error('Failed to get utilization from: %s' % (item['ip']))
            else:
                response.append(
                    utilization
                )
        ...
        if 'state' in view:
            if output == 'default':
                imc_output_handler.print_imc_utilization(response)

            if output == 'compare':
                imc_output_handler.print_imc_utilization_compare(response)
