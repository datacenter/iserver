...
def get_imc_cli_http_command(
    ...
    try:
        ...
        for item in endpoints:
            ...
            http = endpoint_handler.get_http()
            if http is None:
                ctx.my_output.error('Failed to get http from: %s' % (item['ip']))
            else:
                response.append(
                    http
                )
        ...
        if 'state' in view:
            if output == 'default':
                imc_output_handler.print_imc_http(response)
