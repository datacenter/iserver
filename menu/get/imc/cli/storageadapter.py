...
def get_imc_cli_storageadapter_command(
        ctx,
        endpoint_ip,
        username,
        password,
    ...
    """Get imc storage adapter"""

    # iserver get imc storageadapter
    ...
    try:
        ...
        for item in endpoints:
            ...
            storage_adapter = endpoint_handler.get_storage_adapter()
            if storage_adapter is None:
                ctx.my_output.error('Failed to get storageadapter from: %s' % (item['ip']))
            else:
                response = response + storage_adapter
        ...
        if 'state' in view:
            if output == 'default':
                imc_output_handler.print_imc_storage_adapter(response)

            if output == 'compare':
                imc_output_handler.print_imc_storage_adapter_compare(response)

        ctx.my_output.default('Filter: --', before_newline=True)
        ctx.my_output.default('View:   state')
