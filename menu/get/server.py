...
@click.option("--loc", "locator_filter", type=click.Choice(['any', 'on', 'off'], case_sensitive=False), default='any', show_default=True, help="Filter by locator led state")
...
@click.option("--psu", "psu_filter", multiple=True, help="Filter by psu")
...
@click.option("--mem", "memory_filter", default='', callback=validations.validate_int_oper, help="Memory [GiB] filter")
@click.option("--view", "-v", default=['state'], help="[state|adv|alarm|board|boot|connector|contract|cpu|env|fan|fw|gpu|hcl|hw|inv|istate|kvm|lic|mem|net|pci|power|profile|psu|storage|sw|thermal|tpm|workflow|summary]", show_default=True, multiple=True)
...
def get_server_command(
    ...
    view = validations.validate_view(
        ctx,
        view,
        'state|adv|alarm|board|boot|connector|contract|cpu|env|fan|fw|gpu|hcl|hw|inv|istate|kvm|lic|mem|net|pci|power|profile|psu|storage|sw|thermal|tpm|workflow|summary',
        'state',
        [
    ...
    try:
        ...
        settings['memory'] = 'mem' in view
        settings['mac'] = False
        settings['net'] = 'net' in view
        settings['pci'] = 'pci' in view or 'gpu' in view or len(pci_filter) > 0
        settings['power'] = 'power' in view
        settings['profile'] = 'profile' in view
        settings['psu'] = 'psu' in view or len(psu_filter) > 0
        ...
        ctx.my_output.default('Filter: ip, name, serial, model, type, group, loc, power, psu, fan, alarm, ucsm', before_newline=True)
        ctx.my_output.default('        disconnected, standalone, cname, cmodel, cserial, pci, mac, cpu, mem')
        ctx.my_output.default('View:   state (def), adv, alarm, board, boot, connector, contract, cpu|, env, fan, fw, gpu, hcl, hw, inv, istate')
        ctx.my_output.default('        kvm, lic, mem, net, pci, power, profile, psu, storage, sw, thermal, tpm, workflow, summary')
