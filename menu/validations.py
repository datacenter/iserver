...
def validate_apic_controller(ctx, controller_obj, controller_ip, controller_username, controller_password, show_selected=True):
    aci_settings_handler = aci_settings.ApicSettings(log_id=None)
    ...
    if controller_obj is not None:
        ...
        if show_selected and ctx.output == 'default':
            ctx.my_output.default('Apic: %s' % (controller_obj['name']))
...
def validate_apic_controllers(ctx, controller_objs, controller_ip, controller_username, controller_password, show_selected=True):
    aci_settings_handler = aci_settings.ApicSettings(log_id=None)
    ...
    if show_selected and ctx.output == 'default':
        if len(names) > 0:
            ctx.my_output.default('Apic: %s' % (','.join(names)))
...
def validate_apic_controllers_with_nodes(ctx, controller_objs, controller_ip, controller_username, controller_password, node_names, node_role, pod_id=None, user_context=None):
    ...
    if len(apic_handlers) == 1:
        if len(node_names) == 0 and node_role == 'any':
            ...
            node_name = aci_settings_handler.get_default_node(
                apic_handlers[0]['handler'].get_apic_name()
            )
...
def validate_apic_node_names(ctx, apic_handler, node_names, node_role, pod_id=None):
    ...
    if len(nodes_info) == 1:
        ...
        if ctx.output == 'default':
            ctx.my_output.default('Apic: %s' % (apic_handler.get_apic_name()))
            ctx.my_output.default('Pod: %s' % (nodes_info[0]['podId']))
            ctx.my_output.default('Node: %s' % (nodes_info[0]['name']))
...
def validate_ocp_vm_namespace(ctx, ocp_handler, namespace, vm_name):
    if namespace == '':
        vms = ocp_handler.get_ocp_vms_by_name(vm_name)
...
def validate_ocp_vm_yaml_file(ctx, filename):
    ...
    content = validate_yaml_file(ctx, filename)
    if content is None:
        return None
