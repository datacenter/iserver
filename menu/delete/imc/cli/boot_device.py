...
def delete_imc_cli_boot_device_command(
        ctx,
        endpoint_ip,
        username,
        password,
    ...
    try:
        ...
        for item in endpoints:
            ...
            if not endpoint_handler.is_boot_device_name(device_name):
                ctx.my_output.default(
                    'Boot device %s already not defined at %s' % (
                        device_name,
                        item['ip']
                    )
                )
            else:
                success = endpoint_handler.delete_boot_device(
                    device_name
                )
                if not success:
                    ctx.my_output.error('Failed to delete boot device: %s' % (item['ip']))
                    raise ErrorExit

                ctx.my_output.default(
                    'Boot device deleted: %s' % (
                        item['ip']
                    )
                )
