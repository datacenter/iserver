...
def delete_k8s_locvd_command(
        ctx,
        cluster
        ):
    """Delete k8s local volume discovery (ocp)"""
    ...
    try:
        ...
        if k8s_handlers.is_any_local_volume_set(cache_enabled=False):
            ctx.busy = False
            ctx.my_output.error(
                'Local Volume Discovery cannot be deleted when Local Volume Set exists'
            )
            raise ErrorExit
        
        locvds = k8s_handlers.get_local_volume_discoveries()

        ctx.busy = False

        success = k8s_handlers.delete_local_volume_discoveries(
            locvds,
