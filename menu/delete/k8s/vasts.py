...
def delete_k8s_vasts_command(
    ...
    try:
        ...
        if output == 'mo':
            managed_objects = k8s_handlers.get_vast_storages(
        ...
        if 'manifest' in view:
            k8s_output_handler.print_vast_storages_manifest(
                managed_objects
            )

        ctx.my_output.default('Filter: namespace, name', before_newline=True)
        ctx.my_output.default('View:   state (def), manifest')
