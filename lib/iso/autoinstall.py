...
class IsoAutoinstall():
    def __init__(self, template_name, verbose=False, debug=False):
        self.template_name = template_name
        self.my_output = output_helper.OutputHelper(verbose=verbose, debug=debug)
        self.input_validator_handler = input_validator.InputValidator(verbose=verbose, debug=debug)
        self.template_handler = template.Template(verbose=verbose, debug=debug)
        self.verbose = verbose
        self.debug = debug

    def get_script_filename(self):
        template_dir = self.input_validator_handler.get_template_dir(self.template_name)
        if template_dir is None:
            return None

        template_definition = self.input_validator_handler.get_template_definition(self.template_name)
    ...
    def make_iso_remote(self, script_filename, user_data_filename, jump):
        ...
        if not success:
            self.my_output.error('Failed to run dos2unix command')
            return False
        ...
        if not success:
            self.my_output.error('Autoinstall iso generation script failed')
            return False
