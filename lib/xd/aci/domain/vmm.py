...
class AciDomainVmm():
    def __init__(self):
        self.aci_domain_vmm = None

    def load_pre_aci_domain_vmm(self):
        self.aci_domain_vmm = self.get_pre_cache('aci', 'dvmm')
        if self.aci_domain_vmm is None:
            return False
        return True

    def set_post_aci_domain_vmm(self):
        return self.set_post_cache('aci-dvmm', self.aci_domain_vmm)

    def load_post_aci_domain_vmm(self):
        self.aci_domain_vmm = self.get_post_cache('aci-dvmm')
        if self.aci_domain_vmm is None:
            return False
        return True

    def get_aci_domain_vmm(self):
        info = copy.deepcopy(self.aci_domain_vmm)
        return info

    def prepare_aci_domain_vmm(self, cache_enabled=True):
        ...
        for aci_controller in aci_controllers:
            self.my_output.debug('Aci domain vmm: %s' % (aci_controller['name']))
            if cache_enabled and self.cache_ttl is not None:
                # L2-cache
                if aci_controller['name'] in self.aci_domain_vmm:
                    self.my_output.debug('L2 Cache hit')
                    continue

                # L3-cache
                cache = self.get_cache('aci-%s-dvmm' % (aci_controller['name']))
                if cache is not None:
                    self.aci_domain_vmm[aci_controller['name']] = cache
            ...
            apic_domains_vmm = apic_handler.get_domains_vmm(
            ...
            if apic_domains_vmm is None:
                continue

            self.aci_domain_vmm[aci_controller['name']] = []
            for item in apic_domains_vmm:
                item['apic'] = aci_controller['name']
                self.aci_domain_vmm[aci_controller['name']].append(
                    item
                )

            self.set_cache(
                'aci-%s-dvmm' % (aci_controller['name']),
                self.aci_domain_vmm[aci_controller['name']]
            )

        return True

    def run_aci_domain_vmm(self):
        for key in self.aci_domain_vmm:
            for item in self.aci_domain_vmm[key]:
                ...
                item['controllerCount'] = len(item['controller'])
        ...
        if not self.set_post_aci_domain_vmm():
            return False
