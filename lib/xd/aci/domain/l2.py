...
class AciDomainL2():
    def __init__(self):
        self.aci_domain_l2 = None

    def load_pre_aci_domain_l2(self):
        self.aci_domain_l2 = self.get_pre_cache('aci', 'dl2')
        if self.aci_domain_l2 is None:
            return False
        return True

    def set_post_aci_domain_l2(self):
        return self.set_post_cache('aci-dl2', self.aci_domain_l2)

    def load_post_aci_domain_l2(self):
        self.aci_domain_l2 = self.get_post_cache('aci-dl2')
        if self.aci_domain_l2 is None:
            return False
        return True

    def get_aci_domain_l2(self):
        info = copy.deepcopy(self.aci_domain_l2)
        return info

    def prepare_aci_domain_l2(self, cache_enabled=True):
        ...
        for aci_controller in aci_controllers:
            self.my_output.debug('Aci domain l2: %s' % (aci_controller['name']))
            if cache_enabled and self.cache_ttl is not None:
                # L2-cache
                if aci_controller['name'] in self.aci_domain_l2:
                    self.my_output.debug('L2 Cache hit')
                    continue

                # L3-cache
                cache = self.get_cache('aci-%s-dl2' % (aci_controller['name']))
                if cache is not None:
                    self.aci_domain_l2[aci_controller['name']] = cache
            ...
            apic_domains_l2 = apic_handler.get_domains_l2(
                vlan_info=True,
                vlan_usage_info=True,
                node_info=True,
                intf_vlan_info=True
            )
            if apic_domains_l2 is None:
                continue

            self.aci_domain_l2[aci_controller['name']] = []
            for item in apic_domains_l2:
                item['apic'] = aci_controller['name']
                self.aci_domain_l2[aci_controller['name']].append(
                    item
                )

            self.set_cache(
                'aci-%s-dl2' % (aci_controller['name']),
                self.aci_domain_l2[aci_controller['name']]
            )

        return True

    def run_aci_domain_l2(self):
        for key in self.aci_domain_l2:
            for item in self.aci_domain_l2[key]:
        ...
        if not self.set_post_aci_domain_l2():
            return False
