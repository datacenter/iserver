...
class LcmVm():
    ...
    def create(self, vm_parameters, dry_run=False):
        self.vm_parameters = vm_parameters
    ...
    def wait_ssh(self, timeout=3600):
        self.my_output.default('Wait for SSH access with %s seconds timeout' % (timeout))
        self.my_output.info(json.dumps(self.vm_parameters['vm']['ssh'], indent=4))
    ...
    def run_tasks(self):
        if len(self.vm_parameters['vm']['tasks']) == 0:
            self.my_output.info('No tasks defined')
            return True
        ...
        for task_filename in self.vm_parameters['vm']['tasks']:
    ...
    def delete(self, vm_name):
        if not self.is_vm(vm_name):
            self.my_output.info('VM does not exist: %s' % (vm_name))
            return True

        if not self.power_off_vm(vm_name):
            return False
