...
from lib.webex_bot.commands.echo import EchoCommand
from lib.webex_bot.commands.help import HelpCommand
from lib.webex_bot.exceptions import BotException
from lib.webex_bot.formatting import quote_info
from lib.webex_bot.models.command import CALLBACK_KEYWORD_KEY, Command, COMMAND_KEYWORD_KEY
from lib.webex_bot.models.response import Response
from lib.webex_bot.websockets.webex_websocket_client import MyWebexWebsocketClient, DEFAULT_DEVICE_URL
...
class MyWebexBot(MyWebexWebsocketClient):
    ...
    def __init__(self,
        ...
        MyWebexWebsocketClient.__init__(self,
                                      teams_bot_token,
                                      teams_bot_email,
    ...
    def add_command(self, command_class: Command):
        ...
        for c in self.commands:
            ...
            if new_callback_keyword and c.card_callback_keyword == new_callback_keyword:
                raise BaseException(f"Error adding new command: '{command_class.command_keyword}'. "
    ...
    def approval_parameters_check(self):
        ...
        if len(self.approved_users) == 0 and len(self.approved_domains) == 0 and len(self.approved_rooms) == 0:
            log.warning("Your bot is open to anyone on Webex Teams. "
                        ...
                        "MyWebexBot(approved_domains=['example.com'], approved_users=['user@company.com'], "
    ...
    def process_raw_command(self, raw_message, teams_message, user_email, activity, is_card_callback_command=False):
        ...
        message_without_command = MyWebexBot.get_message_passed_to_command(command.command_keyword, raw_message)
