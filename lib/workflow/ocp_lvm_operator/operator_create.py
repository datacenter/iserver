from lib import output_helper
from lib.workflow.ocp_lvm_operator import common as local_common


def validate(params):
    if 'cluster' not in params or params['cluster'] is None:
        return None, 'Cluster name required'

    if 'channel' not in params:
        params['channel'] = '__default__'

    if 'confirmation' not in params:
        params['confirmation'] = True

    if not isinstance(params['confirmation'], bool):
        return None, 'confirmation param must be true or false'

    if 'verbose' not in params:
        params['verbose'] = False

    if not isinstance(params['verbose'], bool):
        return None, 'verbose param must be true or false'
    
    if 'check-verbose' not in params:
        params['check-verbose'] = params['verbose']

    if not isinstance(params['check-verbose'], bool):
        return None, 'check-verbose param must be true or false'
    
    allowed_keys = [
        'cluster',
        'channel',
        'confirmation',
        'verbose',
        'check-verbose'
    ]
    return local_common.sanitize_params(params, allowed_keys), None


def get_namespace_labels():
    labels = {}
    labels['openshift.io/cluster-monitoring'] = 'true'
    labels['pod-security.kubernetes.io/enforce'] = 'privileged'
    labels['pod-security.kubernetes.io/audit'] = 'privileged'
    labels['pod-security.kubernetes.io/warn'] = 'privileged'
    return labels


def run(params, log_id=None):
    my_output = output_helper.OutputHelper(log_id=log_id)
    my_output.default('OpenShift Workflow - LVM Operator - Create Operator', before_newline=True, after_newline=True, double_underline=True)

    params, error = validate(params)
    if error is not None:
        my_output.error(error)
        return False

    params = local_common.initialize(params, my_output, log_id)
    if params is None:
        return False

    if params['k8s_handler'].is_lvm_subscription(params['namespace'], params['name']):
        my_output.default('LVM Operator already created')
        return True

    success = params['k8s_handler'].create_namespace(
        params['namespace'],
        labels=get_namespace_labels(),
        confirmation=params['confirmation'],
        my_output=my_output,
        wait=True
    )
    if not success:
        return False
    
    success = params['k8s_handler'].create_operator_group(
        params['namespace'], 
        name='%s-operatorgroup' % (params['namespace']), 
        add_target_namespaces=True,
        target_namespaces=[params['namespace']], 
        upgrade_strategy=None,
        confirmation=params['confirmation'], 
        my_output=my_output, 
        wait=True
    )
    if not success:
        my_output.error('Operator group create failed')
        return False

    success = params['k8s_handler'].create_lvm_subscription(
        params['namespace'], 
        params['name'],
        params['channel'],
        confirmation=params['confirmation'], 
        my_output=my_output, 
        wait=True            
    )
    if not success:
        return False
    
    my_output.default('')
    my_output.default('Completed tasks')
    my_output.default('- LVM storage operator installed')

    return True
