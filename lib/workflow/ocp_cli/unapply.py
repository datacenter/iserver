...
def run(params, log_id=None):
    ...
    my_output.default('OpenShift Workflow - Delete CRDs from file', before_newline=True, after_newline=True, double_underline=True)
    ...
    for location in params['location']:
        ...
        for filename in contents:
            ...
            if params['namespace'] is None:
                command = 'oc delete -f %s' % (destination)
            else:
                command = 'oc delete -n %s -f %s' % (params['namespace'], destination)
