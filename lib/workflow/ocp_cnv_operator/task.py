...
def validate_create(task, cluster_name, confirmation, cluster_settings=None, k8s_handler=None):
    if not isinstance(task, dict):
        return None, 'cnv task definition in dict format required'
    
    if 'operator' in task:
        if not isinstance(task['operator'], dict):
            return None, 'cnv.operator dict required'
    ...
    if 'instance' in task:
        if not isinstance(task['instance'], dict):
            return None, 'cnv.instance dict required'
    ...
    if len(new_task) == 0:
        return None, 'No valid parameters defined for cnv task'
