...
def validate(params, log_id):
    ...
    if 'ptemplate' not in params or params['ptemplate'] is None:
        params['ptemplate'] = []

    if len(params['ptemplate']) > 0 and params['dir'] is None:
        return None, 'Template directory required'
    
    if 'pmode' not in params or params['pmode'] is None:
        params['pmode'] = 'replace'

    if 'pattribute' not in params or params['pattribute'] is None:
        params['pattribute'] = []

    if 'ptarget' not in params or params['ptarget'] is None:
        params['ptarget'] = []

    settings_handler = settings.OcpSettings(log_id=log_id)
    for target in params['ptarget']:
...
def resolve_moids(params, servers, my_output, log_id):
    ...
    for target in params['ptarget']:
        ...
        if target.split(':')[0] == 'ocp':
            ...
            params['cluster_ids'][cluster_name] = {}
            ...
            for node in nodes:
                ...
                params['cluster_ids'][cluster_name][node['name']] = {}
                params['cluster_ids'][cluster_name][node['name']]['id'] = None
                params['cluster_ids'][cluster_name][node['name']]['device_id'] = None
                for annotation in node['annotations']:
                    if len(annotation.split('intersight-')) > 1:
                        if len(annotation.split('intersight-dev-')) > 1:
                            params['cluster_ids'][cluster_name][node['name']]['device_id'] = node['annotations'][annotation]
                        else:
                            params['cluster_ids'][cluster_name][node['name']]['id'] = node['annotations'][annotation]

                if params['cluster_ids'][cluster_name][node['name']]['id'] is None:
                    my_output.error('node %s has no intersight id annotation' % (node['name']))
                    return None
                
                my_output.default('\tid: %s' % (params['cluster_ids'][cluster_name][node['name']]['id']))

                if params['cluster_ids'][cluster_name][node['name']]['device_id'] is None:
                    my_output.error('node %s has no intersight device id annotation' % (node['name']))
                    return None
                
                my_output.default('\tdevice id: %s' % (params['cluster_ids'][cluster_name][node['name']]['device_id']))

                found = False
                for server in servers:
                    if server['Moid'] == params['cluster_ids'][cluster_name][node['name']]['id']:
                        if server['RegisteredDeviceMoid'] == params['cluster_ids'][cluster_name][node['name']]['device_id']:
                            found = True
...
def generate_advisory_pollers(params):
    content = ''

    for cluster_name in params['cluster_ids']:
        ids = []

        for node_name in params['cluster_ids'][cluster_name]:
            ids.append(params['cluster_ids'][cluster_name][node_name]['id'])

            content = '%s\n[[pollers]]' % (content)
            content = '%s\nname = "intersight.advisory.count"' % (content)
            content = '%s\notel_attributes = { scope = "node:%s" }' % (content, node_name)
            content = '%s\napi_query = "api/v1/tam/AdvisoryInstances?$filter=AffectedObjectMoid eq \'%s\'"' % (
                content, 
                params['cluster_ids'][cluster_name][node_name]['id']
            )
            content = '%s\naggregator = "count_results"' % (content)
            content = '%s\ninterval = 60\n' % (content)

        new_ids = []
        for item in ids:
            new_ids.append('\'%s\'' % (item))

        content = '%s\n[[pollers]]' % (content)
        content = '%s\nname = "intersight.advisory.count"' % (content)
        content = '%s\notel_attributes = { scope = "cluster:%s" }' % (content, cluster_name)
        content = '%s\napi_query = "api/v1/tam/AdvisoryInstances?$filter=AffectedObjectMoid in (%s)"' % (
            content, 
            ', '.join(new_ids)
        )
        content = '%s\naggregator = "count_results"' % (content)
...
def generate_alarm_pollers(params):
    content = ''

    for cluster_name in params['cluster_ids']:
        ids = []
        for node_name in params['cluster_ids'][cluster_name]:
            ids.append(params['cluster_ids'][cluster_name][node_name]['device_id'])

            content = '%s\n[[pollers]]' % (content)
            content = '%s\nname = "intersight.alarm.count"' % (content)
            content = '%s\notel_attributes = { scope = "node:%s", severity = "critical" }' % (content, node_name)
            content = '%s\napi_query = "api/v1/cond/Alarms?$filter=Acknowledge eq \'None\' and Severity eq \'Critical\' and RegisteredDevice/Moid eq \'%s\'&$count=true"' % (
                content, 
                params['cluster_ids'][cluster_name][node_name]['device_id']
            )
            content = '%s\naggregator = "result_count"' % (content)
            content = '%s\ninterval = 300\n' % (content)

            content = '%s\n[[pollers]]' % (content)
            content = '%s\nname = "intersight.alarm.count"' % (content)
            content = '%s\notel_attributes = { scope = "node:%s", severity = "warning" }' % (content, node_name)
            content = '%s\napi_query = "api/v1/cond/Alarms?$filter=Acknowledge eq \'None\' and Severity eq \'Warning\' and RegisteredDevice/Moid eq \'%s\'&$count=true"' % (
                content, 
                params['cluster_ids'][cluster_name][node_name]['device_id']
            )
            content = '%s\naggregator = "result_count"' % (content)
        ...
        content = '%s\n[[pollers]]' % (content)
        content = '%s\nname = "intersight.alarm.count"' % (content)
        content = '%s\notel_attributes = { scope = "cluster:%s", severity = "critical" }' % (content, cluster_name)
        content = '%s\napi_query = "api/v1/cond/Alarms?$filter=Acknowledge eq \'None\' and Severity eq \'Critical\' and RegisteredDevice/Moid in (%s)&$count=true"' % (
            content, 
            ', '.join(new_ids)
        )
        content = '%s\naggregator = "result_count"' % (content)
        ...
        content = '%s\n[[pollers]]' % (content)
        content = '%s\nname = "intersight.alarm.count"' % (content)
        content = '%s\notel_attributes = { scope = "cluster:%s", severity = "warning" }' % (content, cluster_name)
        content = '%s\napi_query = "api/v1/cond/Alarms?$filter=Acknowledge eq \'None\' and Severity eq \'Warning\' and RegisteredDevice/Moid in (%s)&$count=true"' % (
            content, 
            ', '.join(new_ids)
        )
        content = '%s\naggregator = "result_count"' % (content)
...
def generate_poller(params, template_name, my_output):
    ...
    for server_ids in params['server_ids']:
        ...
        pattern = '${%s}' % ('SCOPE')
        content = content.replace(pattern, 'server:%s' % (server_ids['name']))
...
def modify_poller(params, instance, my_output, log_id):
    ...
    for ptemplate in params['ptemplate']:
...
def run(params, log_id=None):
    ...
    for instance in params['instance']:
        ...
        my_output.default('- mode: %s' % (params['pmode']))
        if len(params['ptemplate']) == 0:
            my_output.default('- no template')
        if len(params['ptemplate']) > 0:
            my_output.default('- template: %s' % (','.join(params['ptemplate'])))
            my_output.default('- target: %s' % (','.join(params['ptarget'])))
