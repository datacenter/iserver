...
def validate_create(task, cluster_name, confirmation, cluster_settings=None, k8s_handler=None):
    if not isinstance(task, dict):
        return None, 'odf task definition in dict format required'
    
    if 'operator' in task:
        if not isinstance(task['operator'], dict):
            return None, 'odf.operator dict required'
    ...
    if 'cluster' in task:
        if not isinstance(task['cluster'], dict):
            return None, 'odf.cluster list required'
    ...
    if len(new_task) == 0:
        return None, 'No valid parameters defined for odf task'
