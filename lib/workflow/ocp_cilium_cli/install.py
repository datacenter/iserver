...
from lib.workflow.ocp_cilium_cli import common as local_common
...
def validate(params):
    ...
    if 'url' not in params or params['url'] is None:
        ...
        params['url'] = 'https://github.com/cilium/cilium-cli/releases/download/%s/cilium-linux-amd64.tar.gz' % (version)
...
def run(params, log_id=None):
    ...
    my_output.default('OpenShift Workflow - Install cilium cli', before_newline=True, after_newline=True, double_underline=True)
    ...
    if not params['overwrite']:
        success = params['ssh_handler'].is_file('/usr/local/bin/cilium')
        if success:
            my_output.default('/usr/local/bin/cilium found and overwrite not enforced')
            return True
        
    my_output.default('Downloading cilium binary from %s' % (params['url']))
    ...
    my_output.default('Uploading cilium binary to cluster management node')
    ...
    success, output, error = params['ssh_handler'].run_cmd(
        'sudo chmod a+x /usr/local/bin/cilium'
    )
    if not success:
    ...
    my_output.default('Cilium binary ready to be used')
