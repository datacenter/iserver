...
def validate(params):
    rules = [
        ...
        ['config', False, None, 'file-k8s', None, None, None, None]
    ]
...
def run(params, log_id=None):
    ...
    my_output.default('OpenShift Workflow - OVNKubernetes - Create route advertisement', before_newline=True, after_newline=True, double_underline=True)
    ...
    if not frr_enabled:
        my_output.default('Frr-k8s %s' % (my_output.add_color('not enabled', 'Red')))
        return False
    ...
    success = params['k8s_handler'].create_or_update_route_advertisement(
        file_helper.get_file_yaml(params['config']),
    ...
    my_output.default('- OVN route advertisement created')
