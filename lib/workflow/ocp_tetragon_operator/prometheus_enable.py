...
def enable_service_monitoring(params, my_output):
    ...
    if config['serviceMonitorEnabled'] and config['serviceMonitorEnabled']:
        my_output.default('serviceMonitorEnabled already set to true')
        return True
    
    my_output.default('serviceMonitorEnabled will be set to true')
    if not local_common.update_operator_config(managed_object['data'], params, my_output):
        return False
    
    managed_object['data']['agentDaemonSet'] = managed_object['data']['agentDaemonSet'].replace('serviceMonitorEnabled: false', 'serviceMonitorEnabled: true')
    if not local_common.update_operator_config(managed_object['data'], params, my_output):
        return False
    ...
    success = params['k8s_handler'].wait_service_monitors(service_monitors, my_output=my_output, optional=False)
    if not success:
        return False
...
def run(params, log_id=None):
    ...
    if not enable_service_monitoring(params, my_output):
        return False
    
    my_output.default('')
    my_output.default('Completed tasks')
    my_output.default('- Tetragon service monitors enabled')
