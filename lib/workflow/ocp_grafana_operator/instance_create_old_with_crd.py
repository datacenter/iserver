...
def validate(params):
    ...
    if len(params['crd']) > 0 and isinstance(params['crd'][0], str):
        crd = []
        locations = []
        for policy_item in params['crd']:
            try:
                ...
                return None, 'Dashboard file path detection failed'
    ...
    if not isinstance(params['fixup'], bool):
        return None, 'fixup param must be true or false'
...
def add_crd(params, my_output):
    ...
    for crd in params['crd']:
        ...
        if crd['kind'] != 'GrafanaDashboard':
            my_output.default('Skipping unsupported kind: %s' % (crd['kind']))
            continue

        success = params['k8s_handler'].create_grafana_dashboard_from_yaml(
            params['namespace'], 
            params['instance'], 
            crd, 
            fixup=params['fixup'],
...
def run(params, log_id=None):
    ...
    if not add_crd(params, my_output):
        return False
