...
def edit_config_map(params, config_map_mo, my_output):
    ...
    if 'enableUserWorkload' in data_json and not data_json['enableUserWorkload']:
        my_output.default('- enableUserWorkload already disabled')
        return True, False
    ...
    try:
        ...
        config_data['enableUserWorkload'] = False
...
def disable_user_workload_monitoring(params, my_output):
    ...
    if changed:
        my_output.default('Check for resources gone', before_newline=True, underline=True)
        success = params['k8s_handler'].wait_prometheus_no_user_workload_monitoring(my_output=my_output)
...
def run(params, log_id=None):
    ...
    my_output.default('OpenShift Workflow - Prometheus - Disable user-workload monitoring', before_newline=True, after_newline=True, double_underline=True)
    ...
    if not disable_user_workload_monitoring(params, my_output):
        return False
    
    my_output.default('')
    my_output.default('Completed tasks', underline=True, before_newline=True)
    my_output.default('- User workload monitoring disabled')
