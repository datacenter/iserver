...
def run(params, log_id=None):
    ...
    my_output.default('OpenShift Workflow - Cisco Intersight Operator - Disable UI plugin', before_newline=True, after_newline=True, double_underline=True)
    ...
    if 'intersight-plugin' not in plugins:
        my_output.default('Intersight ui plugin %s' % (my_output.add_color('already disabled', 'Green')))
        return True
    
    body = params['k8s_handler'].copy_managed_object_base(console)
    body['spec']['plugins'] = []
    for item in filter_helper.get(console, 'spec:plugins', on_error=[], on_none=[]):
        if item == 'intersight-plugin':
            continue
        
        body['spec']['plugins'].append(item)
    ...
    my_output.default('- Cisco intersight ui plugin disabled')    
    return True
