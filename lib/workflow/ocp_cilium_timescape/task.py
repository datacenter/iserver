from lib.workflow.ocp_cilium_timescape import feature_enable
...
def validate_create(task, cluster_name, confirmation, cluster_settings=None, k8s_handler=None):
    if 'feature' in task:
        if not isinstance(task['feature'], dict):
            return None, 'cilium_timescape.feature dict required'
    ...
    if len(new_task) == 0:
        return None, 'No valid parameters defined for cilium_timescape task'
