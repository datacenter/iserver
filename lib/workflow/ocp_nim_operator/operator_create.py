...
from lib.workflow.ocp_nim_operator import common as local_common
...
def run(params, log_id=None):
    ...
    my_output.default('OpenShift Workflow - NVIDIA NIM Operator - Create Operator', before_newline=True, after_newline=True, double_underline=True)
    ...
    if subscription is not None:
        my_output.default('NVIDIA NIM operator already created')
    else:
        ...
        success = params['k8s_handler'].create_nim_subscription(
            params['namespace'], 
            params['name'], 
    ...
    my_output.default('- NVIDIA NIM operator installed')
