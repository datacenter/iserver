...
def validate(tasks, cluster_name, cluster_settings=None, k8s_handler=None, confirmation=True):
    ...
    for task in tasks:
        for task_name in task:
            ...
            if task_name == 'cli':
                pass
                # new_task, error = task_cli.validate_delete(
                #     task[task_name],
                #     cluster_name,
                #     confirmation,
            ...
            if task_name == 'cli-web':
                new_task, error = task_cli_web.validate_delete(
                    task[task_name],
                    cluster_name,
                    confirmation,
                    cluster_settings=cluster_settings,
            ...
            if task_name == 'gpu':
                new_task, error = task_gpu.validate_delete(
                    task[task_name],
                    cluster_name,
                    confirmation,
                    cluster_settings=cluster_settings,
            ...
            if task_name == 'serverless':
                new_task, error = task_serverless.validate_delete(
                    task[task_name],
                    cluster_name,
                    confirmation,
                    cluster_settings=cluster_settings,
            ...
            if task_name == 'service-mesh':
                new_task, error = task_service_mesh.validate_delete(
                    task[task_name],
                    cluster_name,
                    confirmation,
                    cluster_settings=cluster_settings,
            ...
            if task_name == 'ai':
                new_task, error = task_ai.validate_delete(
                    task[task_name],
                    cluster_name,
                    confirmation,
                    cluster_settings=cluster_settings,
            ...
            if task_name == 'nim':
                new_task, error = task_nim.validate_delete(
                    task[task_name],
                    cluster_name,
                    confirmation,
                    cluster_settings=cluster_settings,
            ...
            if task_name == 'grafana':
                new_task, error = task_grafana.validate_delete(
                    task[task_name],
                    cluster_name,
                    confirmation,
                    cluster_settings=cluster_settings,
            ...
            if task_name == 'prometheus':
                new_task, error = task_prometheus.validate_delete(
                    task[task_name],
                    cluster_name,
                    confirmation,
                    cluster_settings=cluster_settings,
            ...
            if task_name == 'identity':
                new_task, error = task_identity.validate_delete(
                    task[task_name],
                    cluster_name,
                    confirmation,
                    cluster_settings=cluster_settings,
            ...
            if task_name == 'imm':
                new_task, error = task_imm.validate_delete(
                    task[task_name],
                    cluster_name,
                    confirmation,
                    cluster_settings=cluster_settings,
            ...
            if task_name == 'iotel':
                new_task, error = task_iotel.validate_delete(
                    task[task_name],
                    cluster_name,
                    confirmation,
                    cluster_settings=cluster_settings,
            ...
            if task_name == 'mtv':
                new_task, error = task_mtv.validate_delete(
                    task[task_name],
                    cluster_name,
                    confirmation,
                    cluster_settings=cluster_settings,
            ...
            if task_name == 'cnv':
                new_task, error = task_cnv.validate_delete(
                    task[task_name],
                    cluster_name,
                    confirmation,
                    cluster_settings=cluster_settings,
            ...
            if task_name == 'lso':
                new_task, error = task_lso.validate_delete(
                    task[task_name],
                    cluster_name,
                    confirmation,
                    cluster_settings=cluster_settings,
            ...
            if task_name == 'lvm':
                new_task, error = task_lvm.validate_delete(
                    task[task_name],
                    cluster_name,
                    confirmation,
                    cluster_settings=cluster_settings,
            ...
            if task_name == 'minio':
                new_task, error = task_minio.validate_delete(
                    task[task_name],
                    cluster_name,
                    confirmation,
                    cluster_settings=cluster_settings,
            ...
            if task_name == 'portworx':
                new_task, error = task_portworx.validate_delete(
                    task[task_name],
                    cluster_name,
                    confirmation,
                    cluster_settings=cluster_settings,
            ...
            if task_name == 'nfs':
                new_task, error = task_nfs.validate_delete(
                    task[task_name],
                    cluster_name,
                    confirmation,
                    cluster_settings=cluster_settings,
            ...
            if task_name == 'nmstate':
                new_task, error = task_nmstate.validate_delete(
                    task[task_name],
                    cluster_name,
                    confirmation,
                    cluster_settings=cluster_settings,
            ...
            if task_name == 'odf':
                new_task, error = task_odf.validate_delete(
                    task[task_name],
                    cluster_name,
                    confirmation,
                    cluster_settings=cluster_settings,
            ...
            if task_name == 'sriov':
                new_task, error = task_sriov.validate_delete(
                    task[task_name],
                    cluster_name,
                    confirmation,
                    cluster_settings=cluster_settings,
            ...
            if task_name == 'ssh':
                new_task, error = task_ssh.validate_delete(
                    task[task_name],
                    cluster_name,
                    confirmation,
                    cluster_settings=cluster_settings,
            ...
            if task_name == 'splunk':
                new_task, error = task_splunk.validate_delete(
                    task[task_name],
                    cluster_name,
                    confirmation,
                    cluster_settings=cluster_settings,
            ...
            if task_name == 'tetragon':
                new_task, error = task_tetragon.validate_delete(
                    task[task_name],
                    cluster_name,
                    confirmation,
                    cluster_settings=cluster_settings,
            ...
            if task_name == 'trident':
                new_task, error = task_trident.validate_delete(
                    task[task_name],
                    cluster_name,
                    confirmation,
                    cluster_settings=cluster_settings,
            ...
            if task_name == 'cert-manager':
                new_task, error = task_cert_manager.validate_delete(
                    task[task_name],
                    cluster_name,
                    confirmation,
                    cluster_settings=cluster_settings,
            ...
            if task_name == 'cilium-image':
                pass
                # new_task, error = task_cilium_image.validate_delete(
                #     task[task_name],
                #     cluster_name,
                #     confirmation,
                #     cluster_settings=cluster_settings,
            ...
            if task_name == 'cilium-bgp':
                new_task, error = task_cilium_bgp.validate_delete(
                    task[task_name],
                    cluster_name,
                    confirmation,
                    cluster_settings=cluster_settings,
            ...
            if task_name == 'cilium-timescape':
                new_task, error = task_cilium_timescape.validate_delete(
                    task[task_name],
                    cluster_name,
                    confirmation,
                    cluster_settings=cluster_settings,
            ...
            if task_name == 'cilium-mesh':
                new_task, error = task_cilium_mesh.validate_delete(
                    task[task_name],
                    cluster_name,
                    confirmation,
                    cluster_settings=cluster_settings,
            ...
            if task_name == 'cilium-pnet':
                new_task, error = task_cilium_pnet.validate_delete(
                    task[task_name],
                    cluster_name,
                    confirmation,
                    cluster_settings=cluster_settings,
            ...
            if task_name == 'cilium-inb':
                new_task, error = task_cilium_inb.validate_delete(
                    task[task_name],
                    cluster_name,
                    confirmation,
                    cluster_settings=cluster_settings,
...
def run(tasks, cluster_name, confirmation=True, cluster_settings=None, k8s_handler=None, validate_only=False, break_on_error=True, log_id=None, reverse=True):
    ...
    my_output.default('OpenShift Workflow - Delete Tasks', before_newline=True, after_newline=True, double_underline=True)
    ...
    for task in tasks:
        for task_name in task:
            ...
            if task_name == 'cli-web':
                task_success = task_cli_web.delete(
                    task[task_name],
                    log_id=log_id
                )
                if break_on_error:
            ...
            if task_name == 'serverless':
                task_success = task_serverless.delete(
                    task[task_name],
                    log_id=log_id
                )
                if break_on_error:
            ...
            if task_name == 'service-mesh':
                task_success = task_service_mesh.delete(
                    task[task_name],
                    log_id=log_id
                )
                if break_on_error:
            ...
            if task_name == 'grafana':
                task_success = task_grafana.delete(
                    task[task_name],
                    log_id=log_id
                )
                if break_on_error:
            ...
            if task_name == 'prometheus':
                task_success = task_prometheus.delete(
                    task[task_name],
                    log_id=log_id
                )
                if break_on_error:
            ...
            if task_name == 'identity':
                task_success = task_identity.delete(
                    task[task_name],
                    log_id=log_id
                )
                if break_on_error:
            ...
            if task_name == 'iotel':
                task_success = task_iotel.delete(
                    task[task_name],
                    log_id=log_id
                )
                if break_on_error:
            ...
            if task_name == 'minio':
                task_success = task_minio.delete(
                    task[task_name],
                    log_id=log_id
                )
                if break_on_error:
            ...
            if task_name == 'portworx':
                task_success = task_portworx.delete(
                    task[task_name],
                    log_id=log_id
                )
                if break_on_error:
            ...
            if task_name == 'nmstate':
                task_success = task_nmstate.delete(
                    task[task_name],
                    log_id=log_id
                )
                if break_on_error:
            ...
            if task_name == 'sriov':
                task_success = task_sriov.delete(
                    task[task_name],
                    log_id=log_id
                )
                if break_on_error:
            ...
            if task_name == 'splunk':
                task_success = task_splunk.delete(
                    task[task_name],
                    log_id=log_id
                )
                if break_on_error:
            ...
            if task_name == 'tetragon':
                task_success = task_tetragon.delete(
                    task[task_name],
                    log_id=log_id
                )
                if break_on_error:
            ...
            if task_name == 'trident':
                task_success = task_trident.delete(
                    task[task_name],
                    log_id=log_id
                )
                if break_on_error:
            ...
            if task_name == 'cert-manager':
                task_success = task_cert_manager.delete(
                    task[task_name],
                    log_id=log_id
                )
                if break_on_error:
            ...
            if task_name == 'cilium-image':
                pass
                # task_success = task_cilium_image.delete(
                #     task[task_name],
                #     log_id=log_id
                # )
                # if break_on_error:
            ...
            if task_name == 'cilium-bgp':
                task_success = task_cilium_bgp.delete(
                    task[task_name],
                    log_id=log_id
                )
                if break_on_error:
            ...
            if task_name == 'cilium-timescape':
                task_success = task_cilium_timescape.delete(
                    task[task_name],
                    log_id=log_id
                )
                if break_on_error:
            ...
            if task_name == 'cilium-mesh':
                task_success = task_cilium_mesh.delete(
                    task[task_name],
                    log_id=log_id
                )
                if break_on_error:
            ...
            if task_name == 'cilium-pnet':
                task_success = task_cilium_pnet.delete(
                    task[task_name],
                    log_id=log_id
                )
                if break_on_error:
