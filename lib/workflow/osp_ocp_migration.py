...
def run(ctx, osp_handlers, k8s_handlers, settings, deployment_directory):
    ...
    ctx.my_output.default(
        'Create image snapshot of virtual machine: %s' % (
            osp_virtual_machine_info['name']
        )
    )
    ...
    pvc_namespace = migration_rules['namespace']
    ...
    variables = {}
    variables['NAMESPACE'] = migration_rules['namespace']
    variables['NAME'] = 'migrate-%s' % (migration_tag)
