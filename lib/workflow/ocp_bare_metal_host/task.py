from lib.workflow.ocp_nfd_operator import operator_create
from lib.workflow.ocp_nfd_operator import operator_delete
...
def validate_create(task, cluster_name, confirmation, cluster_settings=None, k8s_handler=None):
    if 'operator' in task:
        if not isinstance(task['operator'], dict):
            return None, 'nfd.operator dict required'
    ...
    if len(new_task) == 0:
        return None, 'No valid parameters defined for nfd task'
