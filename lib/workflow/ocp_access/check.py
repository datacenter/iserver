...
def validate(params, log_id):
    ...
    if 'none-on-error' not in params:
        params['none-on-error'] = True
...
def check_kube_api(params, my_output):
    ...
    if not params['data']['ocp_handler'].k8s_handler.check_api():
    ...
    params['result']['kube'] = True
    params['result']['kubeTick'] = '\u2713'
    params['__Output']['result.kubeTick'] = 'Green'
...
def check_fqdn(params, settings_handler, my_output):
    ...
    params['result']['domain'] = config_info['info']['domain']
    params['result']['api_ip'] = infra_info['api_ip']
    params['result']['api_hostname'] = infra_info['api_hostname']
    ...
    params['result']['fqdnTick'] = '\u2713'
    params['__Output']['result.fqdnTick'] = 'Green'

    if params['verbose']:
        my_output.default('- dns resolution: %s' % (my_output.add_color('ok', 'Green')))
    else:
        my_output.debug('- dns resolution: %s' % (my_output.add_color('ok', 'Green')))
...
def check_ssh(params, settings_handler, my_output):
    ...
    if params['data']['ssh_public_key'] is None:
        if not params['ssh-required']:
            if params['verbose']:
                ...
                my_output.debug('- ssh public key: %s' % (my_output.add_color('undefined', 'Yellow')))
            return params, None
    ...
    params['result']['sshTick'] = '\u2713'
    params['__Output']['result.sshTick'] = 'Green'
    ...
    if params['verbose']:
        ...
        my_output.debug('- cluster node [%s] [key:%s]: %s' % (params['data']['node_ip'], params['data']['ssh_public_key_filename'], my_output.add_color('ok', 'Green')))
...
def check_management(params, settings_handler, my_output):
    ...
    if params['data']['management_ip'] is None:
        if not params['mgmt-required']:
            if params['verbose']:
                ...
                my_output.debug('- management node: %s' % (my_output.add_color('undefined', 'Yellow')))
            return params, None
    ...
    if params['verbose']:
        ...
        my_output.debug('- management node [%s] [key:%s]: %s' % (params['data']['management_ip'], params['data']['ssh_public_key_filename'], my_output.add_color('ok', 'Green')))
    ...
    params['result']['mgmtTick'] = '\u2713'
    params['__Output']['result.mgmtTick'] = 'Green'
...
def check_cli_helm(params, my_output):
    ...
    if params['verbose']:
        ...
        my_output.debug('- cli helm: %s' % my_output.add_color('ok', 'Green'))

    params['result']['helmTick'] = '\u2713'
    params['__Output']['result.helmTick'] = 'Green'
...
def check_cli_virtctl(params, my_output):
    ...
    if not success:
        if not params['cli-virtctl-required']:
            if params['verbose']:
                ...
                my_output.debug('- cli virtctl: %s' % my_output.add_color('not found', 'Yellow'))
            return params, None
    ...
    if params['verbose']:
        ...
        my_output.debug('- cli virtctl: %s' % my_output.add_color('ok', 'Green'))

    params['result']['virtctlTick'] = '\u2713'
    params['__Output']['result.virtctlTick'] = 'Green'
...
def check_cli_cilium(params, my_output):
    ...
    if not success:
        if not params['cli-cilium-required']:
            if params['verbose']:
                ...
                my_output.debug('- cli cilium: %s' % my_output.add_color('not found', 'Yellow'))
            return params, None
    ...
    if params['verbose']:
        ...
        my_output.debug('- cli cilium: %s' % my_output.add_color('ok', 'Green'))

    params['result']['ciliumTick'] = '\u2713'
    params['__Output']['result.ciliumTick'] = 'Green'
...
def check_cli_hubble(params, my_output):
    ...
    if not success:
        if not params['cli-hubble-required']:
            if params['verbose']:
                ...
                my_output.debug('- cli hubble: %s' % my_output.add_color('not found', 'Yellow'))
            return params, None
    ...
    if params['verbose']:
        ...
        my_output.debug('- cli hubble: %s' % my_output.add_color('ok', 'Green'))

    params['result']['hubbleTick'] = '\u2713'
    params['__Output']['result.hubbleTick'] = 'Green'
...
def run(params, log_id=None):
    ...
    if not params['verbose']:
        ...
        if cluster_settings['domain'] is not None:
            ...
            my_output.debug('- cluster: %s' % (my_output.add_color(params['cluster'], 'Blue')))

    params['__Output'] = {}
    params['result'] = {}
    params['result']['kube'] = False
    params['result']['kubeTick'] = '\u2717'
    params['__Output']['result.kubeTick'] = 'Red'
    params['result']['fqdnTick'] = '\u2717'
    params['__Output']['result.fqdnTick'] = 'Red'
    params['result']['sshTick'] = '\u2717'
    params['__Output']['result.sshTick'] = 'Red'
    params['result']['mgmtTick'] = '\u2717'
    params['__Output']['result.mgmtTick'] = 'Red'
    params['result']['ciliumTick'] = '\u2717'
    params['__Output']['result.ciliumTick'] = 'Red'
    params['result']['hubbleTick'] = '\u2717'
    params['__Output']['result.hubbleTick'] = 'Red'
    params['result']['helmTick'] = '\u2717'
    params['__Output']['result.helmTick'] = 'Red'
    params['result']['virtctlTick'] = '\u2717'
    params['__Output']['result.virtctlTick'] = 'Red'
    ...
    if error is not None:
        if not params['helm-cli-required']:
            if params['verbose']:
                ...
                my_output.debug('- helm: not found')
    ...
    if error is not None:
        if not params['virtctl-cli-required']:
            if params['verbose']:
                ...
                my_output.debug('- virtctl: not found')
    ...
    if error is not None:
        if not params['cilium-cli-required']:
            if params['verbose']:
                ...
                my_output.debug('- cilium: not found')
    ...
    if error is not None:
        if not params['hubble-cli-required']:
            if params['verbose']:
                ...
                my_output.debug('- hubble: not found')
