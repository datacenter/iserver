...
def validate(params):
    ...
    if 'size' not in params:
        return None, 'Memory size [Gi] required'
    
    if not isinstance(params['size'], int):
        return None, 'size param must be int'

    if params['size'] < 1 or params['size'] > 32:
        return None, 'size param must be <1, 32> integer'
...
def run(params, log_id=None):
    ...
    my_output.default('OpenShift Workflow - Virtual Machine - Memory Configuration', before_newline=True, after_newline=True, double_underline=True)
    ...
    for virtual_machine in virtual_machines:
        vm_success = params['k8s_handler'].change_virtual_machine_memory(
