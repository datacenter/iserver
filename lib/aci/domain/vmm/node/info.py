class DomainVmmNodeInfo():
    def __init__(self):
        self.domain_vmm_node = {}

    def get_domain_vmm_node_info(self, managed_object):
        info = {}
        info['__Output'] = {}
    ...
    def get_domain_vmm_node(self, domain_name):
        if domain_name in self.domain_vmm_node:
            return self.domain_vmm_node[domain_name]

        # one object or None value is expected
        domain_nodes_mo = self.get_domain_vmm_node_mo(domain_name)
        ...
        self.log.apic_mo(
            'vmmDomP.%s.info' % (domain_name),
            self.domain_vmm_node[domain_name]
        )
