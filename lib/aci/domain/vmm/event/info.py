...
class DomainVmmEventInfo():
    def __init__(self):
        self.domain_vmm_event = None

    def get_domain_vmm_event_info(self, managed_object):
        info = {}
        info['__Output'] = {}
    ...
    def get_domain_vmm_event(self, deduplicate=True):
        if self.domain_vmm_event is not None:
            return self.domain_vmm_event

        managed_objects = self.get_domain_vmm_event_mo()
        if managed_objects is None:
            return None

        self.domain_vmm_event = []
        transaction_ids = []
        for managed_object in managed_objects:
            event_info = self.get_domain_vmm_event_info(
                managed_object
            )
            if not deduplicate or event_info['txId'] not in transaction_ids:
                self.domain_vmm_event.append(
                    event_info
                )
        ...
        self.log.apic_mo(
            'vmmDomP.eventLog.info',
            self.domain_vmm_event
        )

        return self.domain_vmm_event

    def get_domain_vmm_id_event(self, domain_name, event_filter=None):
        events = []

        all_events = self.get_domain_vmm_event()
