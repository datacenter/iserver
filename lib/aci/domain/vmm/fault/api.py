class DomainVmmFaultApi():
    def __init__(self):
        self.domain_vmm_fault_mo = None
        self.domain_vmm_fault_record_mo = None

    def get_domain_vmm_fault_mo(self):
        cache = self.get_object_cache(
            'vmmDomP.fault'
        )
        if cache is not None:
            self.domain_vmm_fault_mo = cache
            self.log.apic_mo(
                'vmmDomP.fault',
                self.domain_vmm_fault_mo
            )
            return self.domain_vmm_fault_mo
        ...
        for managed_object in managed_objects['imdata']:
            ...
            self.domain_vmm_fault_mo.append(
                attributes
            )
    ...
    def get_domain_vmm_fault_record_mo(self):
        cache = self.get_object_cache(
            'vmmDomP.faultRecord'
        )
        if cache is not None:
            self.domain_vmm_fault_record_mo = cache
            self.log.apic_mo(
                'vmmDomP.faultRecord',
                self.domain_vmm_fault_record_mo
            )
            return self.domain_vmm_fault_record_mo
        ...
        if managed_objects is None:
            self.log.error(
                'get_domain_vmm_fault_record_mo',
                'API failed'
            )
            return None
        ...
        for managed_object in managed_objects['imdata']:
            ...
            self.domain_vmm_fault_record_mo.append(
                attributes
            )
