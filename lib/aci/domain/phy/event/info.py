...
class DomainPhyEventInfo():
    def __init__(self):
        self.domain_phy_event = None

    def get_domain_phy_event_info(self, managed_object):
        info = {}
        info['__Output'] = {}
    ...
    def get_domain_phy_event(self, deduplicate=True):
        if self.domain_phy_event is not None:
            return self.domain_phy_event

        managed_objects = self.get_domain_phy_event_mo()
        if managed_objects is None:
            return None

        self.domain_phy_event = []
        transaction_ids = []
        for managed_object in managed_objects:
            event_info = self.get_domain_phy_event_info(
                managed_object
            )
            if not deduplicate or event_info['txId'] not in transaction_ids:
                self.domain_phy_event.append(
                    event_info
                )
        ...
        self.log.apic_mo(
            'physDomP.eventLog.info',
            self.domain_phy_event
        )

        return self.domain_phy_event

    def get_domain_phy_id_event(self, domain_name, event_filter=None):
        events = []

        all_events = self.get_domain_phy_event()
