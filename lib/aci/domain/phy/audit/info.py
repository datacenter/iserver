...
class DomainPhyAuditInfo():
    def __init__(self):
        self.domain_phy_audit = None

    def get_domain_phy_audit_info(self, managed_object):
        info = {}
        info['__Output'] = {}
    ...
    def get_domain_phy_audit(self):
        if self.domain_phy_audit is not None:
            return self.domain_phy_audit

        managed_objects = self.get_domain_phy_audit_mo()
        if managed_objects is None:
            return None

        self.domain_phy_audit = []
        for managed_object in managed_objects:
            audit_info = self.get_domain_phy_audit_info(
                managed_object
            )
            self.domain_phy_audit.append(
                audit_info
            )

        self.log.apic_mo(
            'physDomP.auditRecord.info',
            self.domain_phy_audit
        )

        return self.domain_phy_audit

    def get_domain_phy_id_audit(self, domain_name, audit_filter=None):
        audits = []

        all_audits = self.get_domain_phy_audit()
