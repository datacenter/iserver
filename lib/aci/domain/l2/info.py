...
class DomainL2Info():
            ...
            if managed_object['l2extRtL2DomAtt'] is not None:
                for item in managed_object['l2extRtL2DomAtt']:
                    ...
                    if item['tCl'] == 'l2extOut':
                        ...
                        reln_info['tCl'] = item['tCl']
                        reln_info['tDn'] = item['tDn']
                        reln_info['type'] = 'L2 Out'
                        reln_info['name'] = self.get_l2out_name_from_dn(
                            item['tDn']
                        )
                        info['reln'].append(
                            reln_info
                    ...
                    self.log.error(
                        'get_domain_l2_info',
                        'Unsupported extnwRtL3DomAtt tCl: %s' % (item['tCl'])
                    )
        ...
        for domain_info in all_domains:
            ...
            if vlan_info:
                ...
                if domain_info['vlan'] is not None:
                    ...
                    if domain_info['vlan_info'] is None:
                        self.log.error(
                            'get_domains_l2',
                            'Vlan pool not found: %s' % (domain_info['vlan'])
                        )
            ...
            if node_info:
                domain_node_info = self.get_domain_l2_node(
                    domain_info['name']
                )
            ...
            if fault_info:
                domain_info['faultInst'] = self.get_domain_l2_id_fault(
                    domain_info['name'],
                    'faultInst'
                )

            if hfault_info:
                domain_info['faultRecord'] = self.get_domain_l2_id_fault(
                    domain_info['name'],
                    'faultRecord',
            ...
            if event_info:
                domain_info['eventLog'] = self.get_domain_l2_id_event(
                    domain_info['name'],
                    event_filter=event_filter
                )

            if audit_info:
                domain_info['auditLog'] = self.get_domain_l2_id_audit(
                    domain_info['name'],
    ...
    def get_domain_l2(self, domain_name, vlan_info=False, vlan_usage_info=False):
        domain_filter = ['name:%s' % (domain_name)]
        domains = self.get_domains_l2(
