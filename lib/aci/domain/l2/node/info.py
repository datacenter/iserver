class DomainL2NodeInfo():
    def __init__(self):
        self.domain_l2_node = {}

    def get_domain_l2_node_info(self, managed_object):
        info = {}
        info['__Output'] = {}
    ...
    def get_domain_l2_node(self, domain_name):
        if domain_name in self.domain_l2_node:
            return self.domain_l2_node[domain_name]

        # one object or None value is expected
        domain_nodes_mo = self.get_domain_l2_node_mo(domain_name)
        if domain_nodes_mo is None:
            return None

        self.domain_l2_node[domain_name] = self.get_domain_l2_node_info(
            domain_nodes_mo
        )

        self.log.apic_mo(
            'l2extDomP.%s.info' % (domain_name),
