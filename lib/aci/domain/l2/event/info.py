...
class DomainL2EventInfo():
    def __init__(self):
        self.domain_l2_event = None

    def get_domain_l2_event_info(self, managed_object):
        info = {}
        info['__Output'] = {}
    ...
    def get_domain_l2_event(self, deduplicate=True):
        if self.domain_l2_event is not None:
            return self.domain_l2_event

        managed_objects = self.get_domain_l2_event_mo()
        if managed_objects is None:
            return None

        self.domain_l2_event = []
        transaction_ids = []
        for managed_object in managed_objects:
            event_info = self.get_domain_l2_event_info(
                managed_object
            )
            if not deduplicate or event_info['txId'] not in transaction_ids:
                self.domain_l2_event.append(
                    event_info
                )
        ...
        self.log.apic_mo(
            'l2extDomP.eventLog.info',
            self.domain_l2_event
        )

        return self.domain_l2_event

    def get_domain_l2_id_event(self, domain_name, event_filter=None):
        events = []

        all_events = self.get_domain_l2_event()
