class DomainL2FaultApi():
    def __init__(self):
        self.domain_l2_fault_mo = None
        self.domain_l2_fault_record_mo = None

    def get_domain_l2_fault_mo(self):
        cache = self.get_object_cache(
            'l2extDomP.fault'
        )
        if cache is not None:
            self.domain_l2_fault_mo = cache
            self.log.apic_mo(
                'l2extDomP.fault',
        ...
        for managed_object in managed_objects['imdata']:
            ...
            self.domain_l2_fault_mo.append(
                attributes
            )
    ...
    def get_domain_l2_fault_record_mo(self):
        cache = self.get_object_cache(
            'l2extDomP.faultRecord'
        )
        if cache is not None:
            self.domain_l2_fault_record_mo = cache
            self.log.apic_mo(
                'l2extDomP.faultRecord',
        ...
        for managed_object in managed_objects['imdata']:
            ...
            self.domain_l2_fault_record_mo.append(
                attributes
            )
