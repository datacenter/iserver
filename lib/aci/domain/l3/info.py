...
class DomainL3Info():
            ...
            if managed_object['extnwRtL3DomAtt'] is not None:
                for item in managed_object['extnwRtL3DomAtt']:
                    ...
                    if item['tCl'] == 'l3extOut':
                        ...
                        reln_info['tCl'] = item['tCl']
                        reln_info['tDn'] = item['tDn']
                        reln_info['type'] = 'L3 Out'
                        reln_info['name'] = self.get_l3out_name_from_dn(
                            item['tDn']
                        )
                        info['reln'].append(
                            reln_info
    ...
    def get_domains_l3(
        ...
        for domain_info in all_domains:
            ...
            if node_info:
                domain_node_info = self.get_domain_l3_node(
                    domain_info['name']
                )
            ...
            if fault_info:
                domain_info['faultInst'] = self.get_domain_l3_id_fault(
                    domain_info['name'],
                    'faultInst'
                )

            if hfault_info:
                domain_info['faultRecord'] = self.get_domain_l3_id_fault(
                    domain_info['name'],
                    'faultRecord',
            ...
            if event_info:
                domain_info['eventLog'] = self.get_domain_l3_id_event(
                    domain_info['name'],
                    event_filter=event_filter
                )

            if audit_info:
                domain_info['auditLog'] = self.get_domain_l3_id_audit(
                    domain_info['name'],
