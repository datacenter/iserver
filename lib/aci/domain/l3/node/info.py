class DomainL3NodeInfo():
    def __init__(self):
        self.domain_l3_node = {}

    def get_domain_l3_node_info(self, managed_object):
        info = {}
        info['__Output'] = {}
    ...
    def get_domain_l3_node(self, domain_name):
        if domain_name in self.domain_l3_node:
            return self.domain_l3_node[domain_name]

        # one object or None value is expected
        domain_nodes_mo = self.get_domain_l3_node_mo(domain_name)
        if domain_nodes_mo is None:
            return None

        self.domain_l3_node[domain_name] = self.get_domain_l3_node_info(
            domain_nodes_mo
        )

        self.log.apic_mo(
            'l3extDomP.%s.info' % (domain_name),
            self.domain_l3_node[domain_name]
        )
