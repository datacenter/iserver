...
class DomainL3AuditInfo():
    def __init__(self):
        self.domain_l3_audit = None

    def get_domain_l3_audit_info(self, managed_object):
        info = {}
        info['__Output'] = {}
    ...
    def get_domain_l3_audit(self):
        if self.domain_l3_audit is not None:
            return self.domain_l3_audit

        managed_objects = self.get_domain_l3_audit_mo()
        if managed_objects is None:
            return None

        self.domain_l3_audit = []
        for managed_object in managed_objects:
            audit_info = self.get_domain_l3_audit_info(
                managed_object
            )
            self.domain_l3_audit.append(
                audit_info
            )

        self.log.apic_mo(
            'l3extDomP.auditRecord.info',
            self.domain_l3_audit
        )

        return self.domain_l3_audit

    def get_domain_l3_id_audit(self, domain_name, audit_filter=None):
        audits = []

        all_audits = self.get_domain_l3_audit()
