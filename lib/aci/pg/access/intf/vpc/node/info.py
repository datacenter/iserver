...
class PolicyGroupAccessInterfaceVpcNodeInfo():
    def __init__(self):
        self.policy_group_access_interface_vpc_node = None

    def get_policy_group_access_interface_vpc_node_info(self, managed_object):
        # {
    ...
    def get_policy_groups_access_interface_vpc_node_info(self):
        if self.policy_group_access_interface_vpc_node is not None:
            return self.policy_group_access_interface_vpc_node

        managed_objects = self.get_policy_group_access_interface_vpc_node_mo()
        if managed_objects is None:
            return None

        self.policy_group_access_interface_vpc_node = []
        for managed_object in managed_objects:
            self.policy_group_access_interface_vpc_node.append(
                self.get_policy_group_access_interface_vpc_node_info(
                    managed_object
                )
            )
    ...
    def match_policy_group_access_interface_vpc_node(self, policy_group_info, policy_group_filter):
    ...
    def get_policy_group_access_interface_vpc_nodes(self, policy_group_filter=None, port_info=False):
        all_policy_groups = self.get_policy_groups_access_interface_vpc_node_info()
        ...
        for policy_group_info in all_policy_groups:
            if not self.match_policy_group_access_interface_vpc_node(policy_group_info, policy_group_filter):
                continue
