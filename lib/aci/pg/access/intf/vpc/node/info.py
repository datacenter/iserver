class PolicyGroupAccessInterfaceVpcNodeInfo():
    ...
    def get_policy_group_access_interface_vpc_node(self, policy_name):
        if policy_name in self.policy_group_access_interface_vpc_node:
            return self.policy_group_access_interface_vpc_node[policy_name]

        # one object or None value is expected
        domain_nodes_mo = self.get_policy_group_access_interface_vpc_node_mo(policy_name)
        if domain_nodes_mo is None:
            return None

        self.policy_group_access_interface_vpc_node[policy_name] = self.get_policy_group_access_interface_vpc_node_info(
            domain_nodes_mo
        )

        self.log.apic_mo(
            'infraAccBndlGrp.%s.info' % (policy_name),
            self.policy_group_access_interface_vpc_node[policy_name]
        )
