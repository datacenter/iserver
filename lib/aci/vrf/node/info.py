class VrfNodeInfo():
    ...
    def get_vrf_node_info(self, managed_object):
        info = {}
        info['__Output'] = {}
    ...
    def get_vrf_node(self, tenant_name, vrf_name):
        key = '%s.%s' % (tenant_name, vrf_name)
        if key in self.vrf_node:
            return self.vrf_node[key]

        # one object or None value is expected
        nodes_mo = self.get_vrf_node_mo(tenant_name, vrf_name)
        if nodes_mo is None:
            return None

        self.vrf_node[key] = self.get_vrf_node_info(
            nodes_mo
        )
