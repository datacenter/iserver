...
class ContractStandardEventInfo():
    def __init__(self):
        self.standard_contract_event = None

    def get_standard_contract_event_info(self, managed_object):
        info = {}
        info['__Output'] = {}
    ...
    def get_standard_contract_event(self, deduplicate=True):
        if self.standard_contract_event is not None:
            return self.standard_contract_event

        managed_objects = self.get_standard_contract_event_mo()
        if managed_objects is None:
            return None

        self.standard_contract_event = []
        transaction_ids = []
        for managed_object in managed_objects:
            event_info = self.get_standard_contract_event_info(
                managed_object
            )
            if not deduplicate or event_info['txId'] not in transaction_ids:
                self.standard_contract_event.append(
                    event_info
                )
        ...
        self.log.apic_mo(
            'vzBrCP.eventLog.info',
            self.standard_contract_event
        )

        return self.standard_contract_event

    def get_standard_contract_id_event(self, tenant_name, contract_name, event_filter=None):
        events = []

        all_events = self.get_standard_contract_event()
