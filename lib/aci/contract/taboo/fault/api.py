class ContractTabooFaultApi():
    def __init__(self):
        self.taboo_contract_fault_mo = None
        self.taboo_contract_fault_record_mo = None

    def get_taboo_contract_fault_mo(self):
        cache = self.get_object_cache(
            'vzTaboo.fault'
        )
        if cache is not None:
            self.taboo_contract_fault_mo = cache
            self.log.apic_mo(
                'vzTaboo.fault',
        ...
        for managed_object in managed_objects['imdata']:
            if 'faultInst' in managed_object:
                ...
                self.taboo_contract_fault_mo.append(
                    attributes
                )
    ...
    def get_taboo_contract_fault_record_mo(self):
        cache = self.get_object_cache(
            'vzTaboo.faultRecord'
        )
        if cache is not None:
            self.taboo_contract_fault_record_mo = cache
            self.log.apic_mo(
                'vzTaboo.faultRecord',
                self.taboo_contract_fault_record_mo
            )
            return self.taboo_contract_fault_record_mo
        ...
        if managed_objects is None:
            self.log.error(
                'get_taboo_contract_fault_record_mo',
                'API failed'
            )
            return None
        ...
        for managed_object in managed_objects['imdata']:
            if 'faultRecord' in managed_object:
                ...
                attributes['delegated'] = False
                self.taboo_contract_fault_record_mo.append(
                    attributes
                )
