...
class ContractTabooFaultInfo():
    def __init__(self):
        self.taboo_contract_fault = None
        self.taboo_contract_fault_record = None

    def get_taboo_contract_fault_info(self, managed_object):
        info = {}
        info['__Output'] = {}
    ...
    def get_taboo_contract_fault(self):
        if self.taboo_contract_fault is not None:
            return self.taboo_contract_fault

        managed_objects = self.get_taboo_contract_fault_mo()
        if managed_objects is None:
            return None

        self.taboo_contract_fault = []

        for managed_object in managed_objects:
            fault_info = self.get_taboo_contract_fault_info(
                managed_object
            )
            self.taboo_contract_fault.append(
                fault_info
            )
    ...
    def get_taboo_contract_fault_record(self, deduplicate=True):
        if self.taboo_contract_fault_record is not None:
            return self.taboo_contract_fault_record

        managed_objects = self.get_taboo_contract_fault_record_mo()
        if managed_objects is None:
            return None

        self.taboo_contract_fault_record = []
        fault_ids = []
        ...
        for managed_object in managed_objects:
            ...
            if not deduplicate or fault_info['id'] not in fault_ids:
                self.taboo_contract_fault_record.append(
                    fault_info
                )
                fault_ids.append(
                    fault_info['id']
        ...
        self.log.apic_mo(
            'vzTaboo.faultRecord.info',
            self.taboo_contract_fault_record
        )

        return self.taboo_contract_fault_record

    def get_taboo_contract_id_fault(self, tenant_name, contract_name, fault_object, fault_filter=None):
        faults = []

        if fault_object == 'faultInst':
            all_faults = self.get_taboo_contract_fault()
