...
class InterfacePortChannelFaultInfo():
    def __init__(self):
        self.interface_port_channel_fault = {}
        self.interface_port_channel_fault_record = {}

    def get_interface_port_channel_fault_info(self, managed_object):
        info = {}
        info['__Output'] = {}
    ...
    def get_interface_port_channel_fault(self, pod_id, node_id):
        key = '%s.%s' % (pod_id, node_id)
        if key in self.interface_port_channel_fault:
            return self.interface_port_channel_fault[key]

        managed_objects = self.get_interface_port_channel_fault_mo(pod_id, node_id)
        if managed_objects is None:
            return None

        self.interface_port_channel_fault[key] = []
        for managed_object in managed_objects:
            fault_info = self.get_interface_port_channel_fault_info(
                managed_object
            )
            self.interface_port_channel_fault[key].append(
                fault_info
            )

        self.log.apic_mo(
            'pcAggrIf.fault.info.%s' % (key),
            self.interface_port_channel_fault[key]
        )

        return self.interface_port_channel_fault[key]

    def get_interface_port_channel_fault_record(self, pod_id, node_id, deduplicate=True):
        key = '%s.%s' % (pod_id, node_id)
        if key in self.interface_port_channel_fault_record:
            return self.interface_port_channel_fault_record[key]

        managed_objects = self.get_interface_port_channel_fault_record_mo(pod_id, node_id)
        if managed_objects is None:
            return None
        ...
        for managed_object in managed_objects:
            ...
            if not deduplicate or fault_info['id'] not in fault_ids:
                self.interface_port_channel_fault_record[key].append(
                    fault_info
                )
                fault_ids.append(
                    fault_info['id']
        ...
        self.log.apic_mo(
            'pcAggrIf.faultRecord.info.%s' % (key),
            self.interface_port_channel_fault_record[key]
        )

        return self.interface_port_channel_fault_record[key]

    def get_interface_port_channel_id_fault(self, pod_id, node_id, interface_id, fault_object, fault_filter=None):
        faults = []

        if fault_object == 'faultInst':
            all_faults = self.get_interface_port_channel_fault(pod_id, node_id)
        ...
        if fault_object == 'faultRecord':
            all_faults = self.get_interface_port_channel_fault_record(pod_id, node_id)
            if all_faults is None:
                return faults
