class InterfaceVfcFaultApi():
    def __init__(self):
        self.interface_vfc_fault_mo = {}
        self.interface_vfc_fault_record_mo = {}

    def get_interface_vfc_fault_mo(self, pod_id, node_id):
        key = '%s.%s' % (pod_id, node_id)
        if key in self.interface_vfc_fault_mo:
            return self.interface_vfc_fault_mo[key]
        ...
        if cache is not None:
            self.interface_vfc_fault_mo[key] = cache
            self.log.apic_mo(
                'l2VfcIf.fault.%s' % (key),
                self.interface_vfc_fault_mo[key]
            )
            return self.interface_vfc_fault_mo[key]
        ...
        for managed_object in managed_objects['imdata']:
            ...
            self.interface_vfc_fault_mo[key].append(
                attributes
            )
    ...
    def get_interface_vfc_fault_record_mo(self, pod_id, node_id):
        key = '%s.%s' % (pod_id, node_id)
        if key in self.interface_vfc_fault_record_mo:
            return self.interface_vfc_fault_record_mo[key]
        ...
        if cache is not None:
            self.interface_vfc_fault_record_mo[key] = cache
            self.log.apic_mo(
                'l2VfcIf.faultRecord.%s' % (key),
                self.interface_vfc_fault_record_mo[key]
            )
            return self.interface_vfc_fault_record_mo[key]
        ...
        if managed_objects is None:
            self.log.error(
                'get_interface_vfc_fault_record_mo',
                'API failed'
            )
            return None
        ...
        for managed_object in managed_objects['imdata']:
            ...
            self.interface_vfc_fault_record_mo[key].append(
                attributes
            )
