...
class InterfaceSviEventInfo():
    def __init__(self):
        self.interface_svi_event = {}

    def get_interface_svi_event_info(self, managed_object):
        info = {}
        info['__Output'] = {}
    ...
    def get_interface_svi_event(self, pod_id, node_id, deduplicate=True):
        key = '%s.%s' % (pod_id, node_id)
        if key in self.interface_svi_event:
            return self.interface_svi_event[key]

        managed_objects = self.get_interface_svi_event_mo(pod_id, node_id)
        if managed_objects is None:
            return None

        self.interface_svi_event[key] = []
        transaction_ids = []
        for managed_object in managed_objects:
            event_info = self.get_interface_svi_event_info(
                managed_object
            )
            if not deduplicate or event_info['txId'] not in transaction_ids:
                self.interface_svi_event[key].append(
                    event_info
                )
