class InterfaceSviFaultApi():
    def __init__(self):
        ...
        self.interface_svi_fault_record_mo = {}
    ...
    def get_interface_svi_fault_record_mo(self, pod_id, node_id):
        key = '%s.%s' % (pod_id, node_id)
        if key in self.interface_svi_fault_record_mo:
            return self.interface_svi_fault_record_mo[key]
        ...
        if cache is not None:
            self.interface_svi_fault_record_mo[key] = cache
            self.log.apic_mo(
                'svi.faultRecord.%s' % (key),
                self.interface_svi_fault_record_mo[key]
            )
            return self.interface_svi_fault_record_mo[key]

        class_name = 'topology/pod-%s/node-%s/l2BD' % (pod_id, node_id)
        query = 'rsp-subtree=full&rsp-subtree-class=sviIf&rsp-subtree-include=fault-records,no-scoped,subtree&order-by=faultRecord.created|desc&page=0&page-size=%s' % (self.api_fault_limit)
        managed_objects = self.get_class(
            class_name,
            query=query
        )
        ...
        if managed_objects is None:
            self.log.error(
                'get_interface_svi_fault_record_mo',
                'API failed'
            )
            return None
        ...
        for managed_object in managed_objects['imdata']:
            ...
            self.interface_svi_fault_record_mo[key].append(
                attributes
            )
