class InterfacePhyEeeInfo():
    ...
    def get_interface_phy_eee_info(self, managed_object):
        ...
        # "dn": "topology/pod-1/node-205/sys/phys-[eth1/1]/eeep"
        ...
        if info['interface_id'] is None:
            self.log.error(
                'get_interface_phy_eee_info',
                'Unsupported dn: %s' % (info['dn'])
            )

        return info

    def get_interfaces_phy_eee_info(self, pod_id, node_id):
        key = '%s.%s' % (
            pod_id,
            node_id
        )
        if key in self.interface_phy_eee:
            return self.interface_phy_eee[key]

        managed_objects = self.get_interface_phy_eee_mo(
            pod_id,
            node_id
        )
        ...
        self.interface_phy_eee[key] = []
        for managed_object in managed_objects:
            self.interface_phy_eee[key].append(
                self.get_interface_phy_eee_info(
                    managed_object
                )
            )
    ...
    def get_interface_phy_eee_stats(self, pod_id, node_id, interface_id):
        interfaces_info = self.get_interfaces_phy_eee_info(pod_id, node_id)
