class InterfacePhyCapInfo():
    ...
    def get_interface_phy_cap_info(self, managed_object):
        ...
        if info['interface_id'] is None:
            self.log.error(
                'get_interface_phy_cap_info',
                'Unsupported dn: %s' % (info['dn'])
            )

        return info

    def get_interfaces_phy_cap_info(self, pod_id, node_id):
        key = '%s.%s' % (
            pod_id,
            node_id
        )
        if key in self.interface_phy_cap:
            return self.interface_phy_cap[key]

        managed_objects = self.get_interface_phy_cap_mo(
            pod_id,
            node_id
        )
        ...
        self.interface_phy_cap[key] = []
        for managed_object in managed_objects:
            self.interface_phy_cap[key].append(
                self.get_interface_phy_cap_info(
                    managed_object
                )
            )
    ...
    def get_interface_phy_cap_stats(self, pod_id, node_id, interface_id):
        interfaces_info = self.get_interfaces_phy_cap_info(pod_id, node_id)
