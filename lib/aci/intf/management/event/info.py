...
class InterfaceManagementEventInfo():
    def __init__(self):
        self.interface_management_event = {}

    def get_interface_management_event_info(self, managed_object):
        info = {}
        info['__Output'] = {}
    ...
    def get_interface_management_event(self, pod_id, node_id, deduplicate=True):
        key = '%s.%s' % (pod_id, node_id)
        if key in self.interface_management_event:
            return self.interface_management_event[key]

        managed_objects = self.get_interface_management_event_mo(pod_id, node_id)
        if managed_objects is None:
            return None

        self.interface_management_event[key] = []
        transaction_ids = []
        for managed_object in managed_objects:
            event_info = self.get_interface_management_event_info(
                managed_object
            )
            if not deduplicate or event_info['txId'] not in transaction_ids:
                self.interface_management_event[key].append(
                    event_info
                )
        ...
        self.log.apic_mo(
            'mgmtMgmtIf.eventLog.info.%s' % (key),
            self.interface_management_event[key]
        )

        return self.interface_management_event[key]

    def get_interface_management_id_event(self, pod_id, node_id, interface_id, event_filter=None):
        events = []

        all_events = self.get_interface_management_event(
            pod_id,
            node_id
        )
