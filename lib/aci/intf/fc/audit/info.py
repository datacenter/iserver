...
class InterfaceFcAuditInfo():
    def __init__(self):
        self.interface_fc_audit = {}

    def get_interface_fc_audit_info(self, managed_object):
        info = {}
        info['__Output'] = {}
    ...
    def get_interface_fc_audit(self, pod_id, node_id):
        key = '%s.%s' % (pod_id, node_id)
        if key in self.interface_fc_audit:
            return self.interface_fc_audit[key]

        managed_objects = self.get_interface_fc_audit_mo(
            pod_id,
            node_id
        )
        ...
        self.interface_fc_audit[key] = []
        for managed_object in managed_objects:
            audit_info = self.get_interface_fc_audit_info(
                managed_object
            )
            self.interface_fc_audit[key].append(
                audit_info
            )

        self.log.apic_mo(
            'l1FcPhysIf.auditRecord.info.%s' % (key),
            self.interface_fc_audit[key]
        )

        return self.interface_fc_audit[key]

    def get_interface_fc_id_audit(self, pod_id, node_id, interface_id, audit_filter=None):
        audits = []

        all_audits = self.get_interface_fc_audit(
            pod_id,
            node_id
        )
