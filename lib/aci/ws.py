...
class WebSocket():
    def __init__(self, apic_ip, debug=False):
        self.ws_apic_ip = apic_ip
        self.ws_token = None
        self.ws_monitored_objects = []
        self.ws_timeout = 120
        self.ws_handler = None
        self.ws_subscription_ids = '/tmp/ws_apic_%s.json' % (str(uuid.uuid4()).rsplit('-', maxsplit=1)[-1])
        self.ws_debug = debug
    ...
    def ws_on_open(self, ws_obj):
        ...
        for monitored_object in self.ws_monitored_objects:
            subscription_url = 'https://%s%s.json?subscription=yes&refresh-timeout=%s' % (
                self.ws_apic_ip,
                monitored_object,
                self.ws_timeout
            )
    ...
    def ws_refresh(self):
        while True:
            timeout_epoch = int(time.time()) + self.ws_timeout - 60
            while True:
            ...
            if self.ws_token is None:
                print('[ERROR] APIC token generation')
                return

            print('Subscriptions refresh:')
            for subscription_id in subscription_ids:
                subscription_url = 'https://%s/api/subscriptionRefresh.json?id=%s' % (self.ws_apic_ip, subscription_id)
                response = requests.get(
                    subscription_url,
                    headers={'Cookie': 'APIC-cookie=%s' % (self.ws_token)},
                    verify=False
                )
                if self.ws_debug:
                    print('- GET %s' % (subscription_url))
    ...
    def ws_run(self, objects):
        ...
        self.ws_token = self.get_token(generate_if_none=True)
        if self.ws_token is None:
            print('[ERROR] APIC authentication failed')
            return
        ...
        ws_url = 'wss://%s/socket%s' % (self.ws_apic_ip, self.ws_token)
