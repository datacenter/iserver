class EpgFaultApi():
    def __init__(self):
        self.epg_fault_mo = None
        self.epg_fault_record_mo = None

    def get_epg_fault_mo(self):
        cache = self.get_object_cache(
            'fvAEPg.fault'
        )
        if cache is not None:
            self.epg_fault_mo = cache
            self.log.apic_mo(
                'fvAEPg.fault',
                self.epg_fault_mo
            )
            return self.epg_fault_mo
    ...
    def get_epg_fault_record_mo(self):
        cache = self.get_object_cache(
            'fvAEPg.faultRecord'
        )
        if cache is not None:
            self.epg_fault_record_mo = cache
            self.log.apic_mo(
                'fvAEPg.faultRecord',
                self.epg_fault_record_mo
            )
            return self.epg_fault_record_mo
        ...
        for managed_object in managed_objects['imdata']:
            if 'faultRecord' in managed_object:
                ...
                attributes['delegated'] = False
                self.epg_fault_record_mo.append(
                    attributes
                )
