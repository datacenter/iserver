class PolicyGeneralAaeNodeInfo():
    def __init__(self):
        self.policy_global_aae_node = {}

    def get_policy_global_aae_node_info(self, managed_object):
        info = {}
        info['__Output'] = {}
    ...
    def get_policy_global_aae_node(self, domain_name):
        if domain_name in self.policy_global_aae_node:
            return self.policy_global_aae_node[domain_name]

        # one object or None value is expected
        domain_nodes_mo = self.get_policy_global_aae_node_mo(domain_name)
        if domain_nodes_mo is None:
            return None

        self.policy_global_aae_node[domain_name] = self.get_policy_global_aae_node_info(
            domain_nodes_mo
        )

        self.log.apic_mo(
            'infraAttEntityP.%s.info' % (domain_name),
            self.policy_global_aae_node[domain_name]
        )
