...
class PolicyInterfaceCdpAttachmentInfo():
    def __init__(self):
        self.policy_interface_cdp_attachment = None

    def get_policy_interface_cdp_attachment_info(self, managed_object):
        # "childAction": "",
    ...
    def get_policy_interface_cdp_attachments_info(self):
        if self.policy_interface_cdp_attachment is not None:
            return self.policy_interface_cdp_attachment

        managed_objects = self.get_policy_interface_cdp_attachment_mo()
        if managed_objects is not None:
            self.policy_interface_cdp_attachment = []
            for managed_object in managed_objects:
                self.policy_interface_cdp_attachment.append(
                    self.get_policy_interface_cdp_attachment_info(
                        managed_object
                    )
                )
    ...
    def match_policy_interface_cdp_attachment(self, attachment_info, attachment_filter):
    ...
    def get_policy_interface_cdp_attachments(self, attachment_filter=None):
        all_attachments = self.get_policy_interface_cdp_attachments_info()
        ...
        for attachment_info in all_attachments:
            if not self.match_policy_interface_cdp_attachment(attachment_info, attachment_filter):
                continue
    ...
    def get_policy_interface_cdp_attachments_node_summary(self, attachments):
        nodes = {}
