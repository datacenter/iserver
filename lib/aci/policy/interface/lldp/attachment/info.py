...
class PolicyInterfaceLldpAttachmentInfo():
    def __init__(self):
        self.policy_interface_lldp_attachment = None

    def get_policy_interface_lldp_attachment_info(self, managed_object):
        # "childAction": "",
        # "dn": "topology/pod-1/node-2101/sys/phys-[eth1/33]/rslldpIfPolCons",
        # "forceResolve": "yes",
        # "lcOwn": "local",
    ...
    def get_policy_interface_lldp_attachments_info(self):
        if self.policy_interface_lldp_attachment is not None:
            return self.policy_interface_lldp_attachment

        managed_objects = self.get_policy_interface_lldp_attachment_mo()
        if managed_objects is not None:
            self.policy_interface_lldp_attachment = []
            for managed_object in managed_objects:
                self.policy_interface_lldp_attachment.append(
                    self.get_policy_interface_lldp_attachment_info(
                        managed_object
                    )
                )
    ...
    def match_policy_interface_lldp_attachment(self, attachment_info, attachment_filter):
    ...
    def get_policy_interface_lldp_attachments(self, attachment_filter=None):
        all_attachments = self.get_policy_interface_lldp_attachments_info()
        ...
        for attachment_info in all_attachments:
            if not self.match_policy_interface_lldp_attachment(attachment_info, attachment_filter):
                continue
    ...
    def get_policy_interface_lldp_attachments_node_summary(self, attachments):
        nodes = {}
