class PolicyInterfaceLinkLevelFcContext():
    def __init__(self):
        pass

    def set_policy_interface_link_level_fc_context(self, policies):
        ...
        for policy in policies:
            for interface_info in policy['l1RsQosLlfcIfPolCons']:
        ...
        success = self.context_handler.set(
            'pol-link_level_fc-phy',
            intf_context
        )
        if not success:
            self.log.error(
                'set_policy_interface_link_level_fc_context',
                'Context set failed'
            )
