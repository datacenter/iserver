...
class PolicyInterfaceLinkLevelAttachmentInfo():
    def __init__(self):
        self.policy_interface_link_level_attachment = None

    def get_policy_interface_link_level_attachment_info(self, managed_object):
        info = {}
    ...
    def get_policy_interface_link_level_attachments_info(self):
        if self.policy_interface_link_level_attachment is not None:
            return self.policy_interface_link_level_attachment

        managed_objects = self.get_policy_interface_link_level_attachment_mo()
        if managed_objects is not None:
            self.policy_interface_link_level_attachment = []
            for managed_object in managed_objects:
                self.policy_interface_link_level_attachment.append(
                    self.get_policy_interface_link_level_attachment_info(
                        managed_object
                    )
                )
    ...
    def match_policy_interface_link_level_attachment(self, attachment_info, attachment_filter):
    ...
    def get_policy_interface_link_level_attachments(self, attachment_filter=None):
        all_attachments = self.get_policy_interface_link_level_attachments_info()
        ...
        for attachment_info in all_attachments:
            if not self.match_policy_interface_link_level_attachment(attachment_info, attachment_filter):
                continue
    ...
    def get_policy_interface_link_level_attachments_node_summary(self, attachments):
        nodes = {}
