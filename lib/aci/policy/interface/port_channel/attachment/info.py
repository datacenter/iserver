...
class PolicyInterfacePortChannelAttachmentInfo():
    def __init__(self):
    ...
    def get_policy_interface_port_channel_attachment_info(self, managed_object):
        ...
        info['interfaceId'] = managed_object['dn'].split('aggr-[')[1].split(']')[0]
        info['policyType'] = managed_object['tCl']
        info['policyDn'] = managed_object['tDn']
        info['policyName'] = managed_object['tDn'].split('/')[2][9:]

        return info

    def get_policy_interface_port_channel_attachments_info(self):
        if self.policy_interface_port_channel_attachment is not None:
            return self.policy_interface_port_channel_attachment

        managed_objects = self.get_policy_interface_port_channel_attachment_mo()
        if managed_objects is not None:
            self.policy_interface_port_channel_attachment = []
            for managed_object in managed_objects:
                self.policy_interface_port_channel_attachment.append(
                    self.get_policy_interface_port_channel_attachment_info(
                        managed_object
                    )
                )
    ...
    def match_policy_interface_port_channel_attachment(self, attachment_info, attachment_filter):
    ...
    def get_policy_interface_port_channel_attachments(self, attachment_filter=None):
        all_attachments = self.get_policy_interface_port_channel_attachments_info()
        ...
        for attachment_info in all_attachments:
            if not self.match_policy_interface_port_channel_attachment(attachment_info, attachment_filter):
                continue
    ...
    def get_policy_interface_port_channel_attachments_node_summary(self, attachments):
        nodes = {}
