...
class PolicyInterfaceSlowDrainInfo():
    def __init__(self):
        self.policy_interface_slow_drain = None

    def get_policy_interface_slow_drain_reln_info(self, managed_object):
        info = {}
        info['class'] = 'qosSdIfPol'
    ...
    def get_policy_interface_slow_drain_info(self, managed_object):
        ...
        for reln_mo in managed_object['relnFrom']:
            info['relnFrom'].append(
                self.get_policy_interface_slow_drain_reln_info(
                    reln_mo
                )
            )
    ...
    def get_policies_interface_slow_drain_info(self):
        if self.policy_interface_slow_drain is not None:
            return self.policy_interface_slow_drain

        managed_objects = self.get_policy_interface_slow_drain_mo()
        if managed_objects is not None:
            self.policy_interface_slow_drain = []
            for managed_object in managed_objects:
                self.policy_interface_slow_drain.append(
                    self.get_policy_interface_slow_drain_info(
                        managed_object
                    )
                )
    ...
    def match_policy_interface_slow_drain(self, policy_info, policy_filter):
        ...
        for ap_rule in policy_filter:
            ...
            if key == 'node':
                if 'l1RsQosSdIfPolCons' in policy_info:
                    if len(policy_info['l1RsQosSdIfPolCons']) == 0:
                        return False
    ...
    def match_policy_interface_slow_drain_reln(self, policy_reln_info, policy_filter):
        if policy_filter is None or len(policy_filter) == 0:
            return True
    ...
    def get_policy_interface_slow_drain(self, policy_filter=None, reln_info=True, attachment_info=False):
        ...
        for policy_info in all_policies:
            if not self.match_policy_interface_slow_drain(policy_info, policy_filter):
                continue
            ...
            if attachment_info:
                ...
                policy_info['l1RsQosSdIfPolCons'] = self.get_policy_interface_slow_drain_attachments(
                    attachment_filter=attachment_filter
                )

                policy_info['interfaces'] = 0
                if policy_info['l1RsQosSdIfPolCons'] is not None:
                    policy_info['interfaces'] = len(
                        policy_info['l1RsQosSdIfPolCons']
                    )

                policy_info['nodeInterfaces'] = self.get_policy_interface_slow_drain_attachments_node_summary(
                    policy_info['l1RsQosSdIfPolCons']
                )
