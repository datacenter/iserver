...
class PolicyInterfaceSlowDrainAttachmentInfo():
    def __init__(self):
    ...
    def get_policy_interface_slow_drain_attachments_info(self):
        if self.policy_interface_slow_drain_attachment is not None:
            return self.policy_interface_slow_drain_attachment

        managed_objects = self.get_policy_interface_slow_drain_attachment_mo()
        if managed_objects is not None:
            self.policy_interface_slow_drain_attachment = []
            for managed_object in managed_objects:
                self.policy_interface_slow_drain_attachment.append(
                    self.get_policy_interface_slow_drain_attachment_info(
                        managed_object
                    )
                )
    ...
    def match_policy_interface_slow_drain_attachment(self, attachment_info, attachment_filter):
    ...
    def get_policy_interface_slow_drain_attachments(self, attachment_filter=None):
        all_attachments = self.get_policy_interface_slow_drain_attachments_info()
        ...
        for attachment_info in all_attachments:
            if not self.match_policy_interface_slow_drain_attachment(attachment_info, attachment_filter):
                continue
    ...
    def get_policy_interface_slow_drain_attachments_node_summary(self, attachments):
        nodes = {}
