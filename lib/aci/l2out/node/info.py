class L2OutNodeInfo():
    ...
    def get_l2out_node_info(self, managed_object):
        info = {}
        info['__Output'] = {}
    ...
    def get_l2out_node(self, tenant_name, l2out_name):
        key = '%s.%s' % (tenant_name, l2out_name)
        if key in self.l2out_node:
            return self.l2out_node[key]

        # one object or None value is expected
        nodes_mo = self.get_l2out_node_mo(tenant_name, l2out_name)
        if nodes_mo is None:
            return None

        self.l2out_node[key] = self.get_l2out_node_info(
            nodes_mo
        )
