class L2OutEventApi():
    def __init__(self):
        self.l2out_event_mo = None

    def get_l2out_event_mo(self):
        if self.l2out_event_mo is not None:
            return self.l2out_event_mo

        cache = self.get_object_cache(
            'l2extOut.eventLog'
        )
        if cache is not None:
            self.l2out_event_mo = cache
            self.log.apic_mo(
                'l2extOut.eventLog',
                self.l2out_event_mo
            )
