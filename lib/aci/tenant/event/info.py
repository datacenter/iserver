...
class TenantEventInfo():
    def __init__(self):
        self.tenant_event = None

    def get_tenant_event_info(self, managed_object):
        info = {}
        info['__Output'] = {}
    ...
    def get_tenant_event(self, deduplicate=True):
        if self.tenant_event is not None:
            return self.tenant_event

        managed_objects = self.get_tenant_event_mo()
        ...
        for managed_object in managed_objects:
            event_info = self.get_tenant_event_info(
                managed_object
            )
            if not deduplicate or event_info['txId'] not in transaction_ids:
                self.tenant_event.append(
                    event_info
                )
        ...
        self.log.apic_mo(
            'fvTenant.eventLog.info',
            self.tenant_event
        )

        return self.tenant_event

    def get_tenant_id_event(self, tenant_name, event_filter=None):
        events = []

        all_events = self.get_tenant_event()
        if all_events is None:
            return events

        for event_info in all_events:
            if event_info['tenantName'] is not None:
                if event_info['tenantName'] == tenant_name:
