...
class ApplicationProfileFaultInfo():
    def __init__(self):
        self.application_profile_fault = None
        self.application_profile_fault_record = None

    def get_application_profile_fault_info(self, managed_object):
        info = {}
        info['__Output'] = {}
    ...
    def get_application_profile_fault(self):
        if self.application_profile_fault is not None:
            return self.application_profile_fault

        managed_objects = self.get_application_profile_fault_mo()
        if managed_objects is None:
            return None

        self.application_profile_fault = []

        for managed_object in managed_objects:
            fault_info = self.get_application_profile_fault_info(
                managed_object
            )
            self.application_profile_fault.append(
                fault_info
            )
    ...
    def get_application_profile_fault_record(self, deduplicate=True):
        if self.application_profile_fault_record is not None:
            return self.application_profile_fault_record

        managed_objects = self.get_application_profile_fault_record_mo()
        if managed_objects is None:
            return None

        self.application_profile_fault_record = []
        fault_ids = []
        ...
        for managed_object in managed_objects:
            ...
            if not deduplicate or fault_info['id'] not in fault_ids:
                self.application_profile_fault_record.append(
                    fault_info
                )
                fault_ids.append(
                    fault_info['id']
        ...
        self.log.apic_mo(
            'fvAp.faultRecord.info',
            self.application_profile_fault_record
        )

        return self.application_profile_fault_record

    def get_application_profile_id_fault(self, tenant_name, ap_name, fault_object, fault_filter=None):
        faults = []

        if fault_object == 'faultInst':
            all_faults = self.get_application_profile_fault()
        ...
        for fault_info in all_faults:
            if fault_info['tenantName'] is not None and fault_info['apName'] is not None:
                if fault_info['tenantName'] == tenant_name and fault_info['apName'] == ap_name:
