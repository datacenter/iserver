class ApplicationProfileNodeInfo():
    def __init__(self):
        self.application_profile_node = {}

    def get_application_profile_node_info(self, managed_object):
        info = {}
        info['__Output'] = {}
    ...
    def get_application_profile_node(self, tenant_name, ap_name):
        key = '%s.%s' % (tenant_name, ap_name)
        if key in self.application_profile_node:
            return self.application_profile_node[key]

        # one object or None value is expected
        nodes_mo = self.get_application_profile_node_mo(tenant_name, ap_name)
        if nodes_mo is None:
            return None
        ...
        self.log.apic_mo(
            'fvAp.%s.info' % (key),
            self.application_profile_node[key]
        )
