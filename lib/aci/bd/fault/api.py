class BridgeDomainFaultApi():
    def __init__(self):
        self.bridge_domain_fault_mo = None
        self.bridge_domain_fault_record_mo = None

    def get_bridge_domain_fault_mo(self):
        cache = self.get_object_cache(
            'fvBD.fault'
        )
        if cache is not None:
            self.bridge_domain_fault_mo = cache
            self.log.apic_mo(
                'fvBD.fault',
        ...
        for managed_object in managed_objects['imdata']:
            if 'faultInst' in managed_object:
                ...
                self.bridge_domain_fault_mo.append(
                    attributes
                )
    ...
    def get_bridge_domain_fault_record_mo(self):
        ...
        if cache is not None:
            self.bridge_domain_fault_record_mo = cache
            self.log.apic_mo(
                'fvBD.faultRecord',
                self.bridge_domain_fault_record_mo
            )
            return self.bridge_domain_fault_record_mo
        ...
        if managed_objects is None:
            self.log.error(
                'get_bridge_domain_fault_record_mo',
                'API failed'
            )
            return None
        ...
        for managed_object in managed_objects['imdata']:
            if 'faultRecord' in managed_object:
                ...
                attributes['delegated'] = False
                self.bridge_domain_fault_record_mo.append(
                    attributes
                )
