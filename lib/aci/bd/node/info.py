class BridgeDomainNodeInfo():
    def __init__(self):
        self.bridge_domain_node = {}

    def get_bridge_domain_node_info(self, managed_object):
        info = {}
        info['__Output'] = {}
    ...
    def get_bridge_domain_node(self, tenant_name, ap_name):
        key = '%s.%s' % (tenant_name, ap_name)
        if key in self.bridge_domain_node:
            return self.bridge_domain_node[key]

        # one object or None value is expected
        nodes_mo = self.get_bridge_domain_node_mo(tenant_name, ap_name)
        if nodes_mo is None:
            return None

        self.bridge_domain_node[key] = self.get_bridge_domain_node_info(
            nodes_mo
        )

        self.log.apic_mo(
            'fvBD.%s.info' % (key),
            self.bridge_domain_node[key]
        )
