...
class BridgeDomainAuditInfo():
    def __init__(self):
        self.bridge_domain_audit = None

    def get_bridge_domain_audit_info(self, managed_object):
        info = {}
        info['__Output'] = {}
    ...
    def get_bridge_domain_audit(self):
        if self.bridge_domain_audit is not None:
            return self.bridge_domain_audit

        managed_objects = self.get_bridge_domain_audit_mo()
        if managed_objects is None:
            return None

        self.bridge_domain_audit = []
        for managed_object in managed_objects:
            audit_info = self.get_bridge_domain_audit_info(
                managed_object
            )
            self.bridge_domain_audit.append(
                audit_info
            )

        self.log.apic_mo(
            'fvBD.auditRecord.info',
            self.bridge_domain_audit
        )

        return self.bridge_domain_audit

    def get_bridge_domain_id_audit(self, tenant_name, bd_name, audit_filter=None):
        audits = []

        all_audits = self.get_bridge_domain_audit()
        if all_audits is None:
            return audits

        for audit_info in all_audits:
            if audit_info['tenantName'] is not None and audit_info['bdName'] is not None:
                if audit_info['tenantName'] == tenant_name and audit_info['bdName'] == bd_name:
