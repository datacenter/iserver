...
class ProtocolHsrpEventInfo():
    def __init__(self):
        self.proto_hsrp_event = {}

    def get_protocol_hsrp_event_info(self, managed_object):
        info = {}
        info['__Output'] = {}
        for key in managed_object:
            info[key] = managed_object[key]

        # topology/pod-1/node-2205/sys/hsrp/inst
    ...
    def get_protocol_hsrp_event(self, pod_id, node_id):
        key = '%s.%s' % (pod_id, node_id)
        if key in self.proto_hsrp_event:
            return self.proto_hsrp_event[key]

        managed_objects = self.get_protocol_hsrp_event_mo(pod_id, node_id)
        if managed_objects is None:
            return None

        self.proto_hsrp_event[key] = []
        for managed_object in managed_objects:
            self.proto_hsrp_event[key].append(
                self.get_protocol_hsrp_event_info(
                    managed_object
                )
            )

        self.log.apic_mo(
            'hsrp.eventLog.info.%s' % (key),
            self.proto_hsrp_event[key]
        )
