class ProtocolBfdFaultApi():
    ...
    def get_protocol_bfd_fault_record_mo(self, pod_id, node_id):
        key = '%s.%s' % (pod_id, node_id)
        if key in self.bfd_fault_record_mo:
            return self.bfd_fault_record_mo[key]
        ...
        if cache is not None:
            self.bfd_fault_record_mo[key] = cache
            self.log.apic_mo(
                'bfdInst.faultRecord.%s' % (key),
                self.bfd_fault_record_mo[key]
            )
            return self.bfd_fault_record_mo[key]
        ...
        if managed_objects is None:
            self.log.error(
                'get_protocol_bfd_fault_record_mo',
                'API failed'
            )
            return None
        ...
        for managed_object in managed_objects['imdata']:
            ...
            self.bfd_fault_record_mo[key].append(
                attributes
            )
