...
class ProtocolIpv6EventInfo():
    def __init__(self):
        self.proto_ipv6_event = {}

    def get_protocol_ipv6_event_info(self, managed_object):
        info = {}
        info['__Output'] = {}
        for key in managed_object:
            info[key] = managed_object[key]

        # topology/pod-1/node-2208/sys/ipv6/inst/dom-common:Infra_privIP_VRF/if-[vlan50]/addr-[169.169.170.254/24]
    ...
    def get_protocol_ipv6_event(self, pod_id, node_id):
        key = '%s.%s' % (pod_id, node_id)
        if key in self.proto_ipv6_event:
            return self.proto_ipv6_event[key]

        managed_objects = self.get_protocol_ipv6_event_mo(pod_id, node_id)
        if managed_objects is None:
            return None

        self.proto_ipv6_event[key] = []
        for managed_object in managed_objects:
            self.proto_ipv6_event[key].append(
                self.get_protocol_ipv6_event_info(
                    managed_object
                )
            )

        self.log.apic_mo(
            'ipv6Dom.eventLog.info.%s' % (key),
            self.proto_ipv6_event[key]
        )
