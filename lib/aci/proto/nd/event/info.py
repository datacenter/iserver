...
class ProtocolNdEventInfo():
    def __init__(self):
        self.proto_nd_event = {}

    def get_protocol_nd_event_info(self, managed_object):
        info = {}
        info['__Output'] = {}
    ...
    def get_protocol_nd_event(self, pod_id, node_id):
        key = '%s.%s' % (pod_id, node_id)
        if key in self.proto_nd_event:
            return self.proto_nd_event[key]

        managed_objects = self.get_protocol_nd_event_mo(pod_id, node_id)
        if managed_objects is None:
            return None

        self.proto_nd_event[key] = []
        for managed_object in managed_objects:
            self.proto_nd_event[key].append(
                self.get_protocol_nd_event_info(
                    managed_object
                )
            )

        self.log.apic_mo(
            'ndDom.eventLog.info.%s' % (key),
            self.proto_nd_event[key]
        )
