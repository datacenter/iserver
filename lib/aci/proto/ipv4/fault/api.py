class ProtocolIpv4FaultApi():
    def __init__(self):
        self.proto_ipv4_fault_mo = {}
        self.proto_ipv4_fault_record_mo = {}

    def get_protocol_ipv4_fault_mo(self, pod_id, node_id):
        key = '%s.%s' % (pod_id, node_id)
        if key in self.proto_ipv4_fault_mo:
            return self.proto_ipv4_fault_mo[key]
        ...
        if cache is not None:
            self.proto_ipv4_fault_mo[key] = cache
            self.log.apic_mo(
                'ipv4Dom.fault.%s' % (key),
                self.proto_ipv4_fault_mo[key]
            )
            return self.proto_ipv4_fault_mo[key]
        ...
        for managed_object in managed_objects['imdata']:
            ...
            self.proto_ipv4_fault_mo[key].append(
                attributes
            )
    ...
    def get_protocol_ipv4_fault_record_mo(self, pod_id, node_id):
        key = '%s.%s' % (pod_id, node_id)
        if key in self.proto_ipv4_fault_record_mo:
            return self.proto_ipv4_fault_record_mo[key]
        ...
        if cache is not None:
            self.proto_ipv4_fault_record_mo[key] = cache
            self.log.apic_mo(
                'ipv4Dom.faultRecord.%s' % (key),
                self.proto_ipv4_fault_record_mo[key]
            )
            return self.proto_ipv4_fault_record_mo[key]
        ...
        if managed_objects is None:
            self.log.error(
                'get_protocol_ipv4_fault_record_mo',
                'API failed'
            )
            return None
        ...
        for managed_object in managed_objects['imdata']:
            ...
            self.proto_ipv4_fault_record_mo[key].append(
                attributes
            )
