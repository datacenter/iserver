...
class ProtocolIpv4RouteInfo():
    def __init__(self):
        self.ipv4_routes = {}

    def get_protocol_ipv4_route_next_hop_types(self, route):
        next_hop_types = []
    ...
    def get_protocol_ipv4_routes_summary(self, routes):
        summary = {}
        summary['count'] = 0
        ...
        for route in routes:
            ...
            next_hop_types = self.get_protocol_ipv4_route_next_hop_types(route)
    ...
    def check_protocol_ipv4_route_filter_match(self, ipv4_route_info, route_filter):
        ...
        for rule in route_filter:
            if rule in ['ibgp', 'ebgp', 'static', 'local', 'direct']:
                match = False
                if 'next_hop' in ipv4_route_info:
                    for next_hop in ipv4_route_info['next_hop']:
            ...
            if rule.startswith('ip:'):
                ...
                if ipv4_route_info['prefix'] != '0.0.0.0/0':
                    if not ip_helper.is_ipv4_in_cidr(ip_address_search, ipv4_route_info['prefix']):
                        return False

            if rule.startswith('subnet:'):
                ip_subnet_search = rule.split(':')[1]
                if ip_subnet_search != ipv4_route_info['prefix']:
                    return False
    ...
    def get_protocol_ipv4_route_info(self, managed_object):
        info = {}
        info['__Output'] = {}
    ...
    def get_protocol_ipv4_routes_info(self, pod_id, node_id, ipv4_domain_name):
        ...
        ipv4_routes_mo = self.get_protocol_ipv4_routes_mo(pod_id, node_id, ipv4_domain_name)
        if ipv4_routes_mo is not None:
            self.ipv4_routes[key] = []
            for hsrp_interface_mo in ipv4_routes_mo:
                self.ipv4_routes[key].append(
                    self.get_protocol_ipv4_route_info(
                        hsrp_interface_mo
                    )
                )

        self.log.apic_mo(
            'uribv4Route.info.%s' % (key),
            self.ipv4_routes[key]
        )

        return self.ipv4_routes[key]

    def get_protocol_ipv4_routes(self, pod_id, node_id, ipv4_domain_name, route_filter=None):
        all_routes = self.get_protocol_ipv4_routes_info(
            pod_id,
            node_id,
            ipv4_domain_name
        )
        ...
        for ipv4_route_info in all_routes:
            if ipv4_route_info['prefix'] == '0.0.0.0/0':
            ...
            if route_filter is not None:
                if not self.check_protocol_ipv4_route_filter_match(ipv4_route_info, route_filter):
                    continue
