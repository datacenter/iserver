class MdAciDomainL2Output():
    def __init__(self):
        pass

    def print_aci_domain_l2_details(self, info):
        self.print_page_header('ACI Domain L2 - External Bridge Domain')
        self.my_output.print_stream('- Controller: [%s](../%s-domain-l2.md)' % (info['apic'], info['apic']), 'output')
    ...
    def print_aci_domain_l2(self, info, controller):
        self.print_page_header('Domain L2 - External Bridge Domain (%s)' % (controller))
        self.print_aci_controller_bar(controller, 'domain-l2')
        self.print_aci_policy_table_bar(controller, 'domain-l2')
        ...
        for item in info:
            ...
            self.aci_domain_l2_count[controller] = self.aci_domain_l2_count[controller] + 1

        self.save_output('%s-domain-l2' % (controller), subdir='apic')

        for item in info:
            self.print_aci_domain_l2_details(item)
