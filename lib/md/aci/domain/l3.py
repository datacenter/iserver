class MdAciDomainL3Output():
    def __init__(self):
        pass

    def print_aci_domain_l3_details(self, info):
        self.print_page_header('ACI Domain L3')
        self.my_output.print_stream('- Controller: [%s](../%s-domain-l3.md)' % (info['apic'], info['apic']), 'output')
        ...
        if len(info['l3out']) == 0:
            self.my_output.print_stream('- Associated L3Out: ---', 'output')
        else:
            self.my_output.print_stream('- Associated L3Out', 'output')
            for l3out_info in info['l3out']:
                self.my_output.print_stream(
                    '\t- [%s](../l3out/%s.md)' % (
                        l3out_info['name'],
                        l3out_info['hash']
    ...
    def print_aci_domain_l3(self, info, controller):
        self.print_page_header('Domain L3 (%s)' % (controller))
        self.print_aci_controller_bar(controller, 'domain-l3')
        self.print_aci_policy_table_bar(controller, 'domain-l3')
        ...
        for item in info:
            ...
            self.aci_domain_l3_count[controller] = self.aci_domain_l3_count[controller] + 1

        self.save_output('%s-domain-l3' % (controller), subdir='apic')

        for item in info:
            self.print_aci_domain_l3_details(item)
