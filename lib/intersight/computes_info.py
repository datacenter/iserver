...
class ComputesInfo(ComputeFilter, ComputesWorkflow, ComputesSummary):
    ...
    def get_server_info(self, server, log_id=None, state_enabled=True, cache_enabled=False, include_object=False):
        try:
    ...
    def prepare_cache(self, base_servers, cache_enabled):
        ...
        for base_server in base_servers:
            ...
            if cache_enabled:
                server_cache_properties = self.cache_handler.get_intersight_cache_entry_properties(
                    base_server['Serial']
                )
                if server_cache_properties is not None:
                    for server_cache_property in server_cache_properties:
                        if server_cache_property not in cache_properties:
                            cache_properties[server_cache_property] = 0

                        cache_properties[server_cache_property] = cache_properties[server_cache_property] + 1
        ...
        if self.settings['pci']:
            if cache_enabled:
                ...
                if 'PciDevicesInfo' not in cache_properties or cache_properties['PciDevicesInfo'] < len(base_servers):
                    cache_populated = False

                if 'PciModels' not in cache_properties or cache_properties['PciModels'] < len(base_servers):
                    cache_populated = False
    ...
    def get_threaded(self, base_servers, match_rules, state_enabled, cache_enabled, include_object=False):
        start_time = int(time.time() * 1000)
        servers = []
        ...
        with ProcessPoolExecutor() as executor:
            pool = [executor.submit(self.get_server_info, server, log_id=self.log_id, state_enabled=state_enabled, cache_enabled=cache_enabled, include_object=include_object) for server in base_servers]
            result = concurrent.futures.wait(
                pool,
                timeout=120,
                return_when=concurrent.futures.ALL_COMPLETED
    ...
    def get_sequential(self, base_servers, match_rules, state_enabled, cache_enabled, include_object=False):
        start_time = int(time.time() * 1000)
        servers = []
    ...
    def get(self, match_rules=None, parallel=True, include_object=False, inventory_cache_enabled=False, cache_enabled=False, state_enabled=True):
        ...
        if self.settings['rack']:
            if inventory_cache_enabled:
                rack_servers = self.cache_handler.get_intersight_cache_entry_property(
        ...
        if self.settings['blade']:
            if inventory_cache_enabled:
                blade_servers = self.cache_handler.get_intersight_cache_entry_property(
