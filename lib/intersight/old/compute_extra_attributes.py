...
class ComputeExtraAttributes():
    ...
    def set_intersight_cache(self, key, server_moids, device_moids, registration_moids, board_moids, adapter_moids, boot_moids, serial, workflow_seconds, filter_length_threshold=20, cache_ttl=None):
        # if key == 'locator_led':
        ...
        if key == 'board':
            ...
            if len(target_moids) < filter_length_threshold:
                ...
                self.log.debug(
                    'compute_info.set_intersight_cache',
                    'board cache miss w/filter: %s' % (target_moids)
                )
            else:
                self.log.debug(
                    'compute_info.set_intersight_cache',
        ...
        if key == 'boot_bios':
            ...
            if len(target_moids) < filter_length_threshold:
                ...
                self.log.debug(
                    'compute_info.set_intersight_cache',
                    'boot bios cache miss w/filter: %s' % (target_moids)
                )
            else:
                self.log.debug(
                    'compute_info.set_intersight_cache',
        ...
        if key == 'boot_mode':
            ...
            if len(target_moids) < filter_length_threshold:
                ...
                self.log.debug(
                    'compute_info.set_intersight_cache',
                    'boot mode cache miss w/filter: %s' % (target_moids)
                )
            else:
                self.log.debug(
                    'compute_info.set_intersight_cache',
    ...
    def get_server_info(self, server_mo, settings):
        ...
        self.server_info['CacheHit'] = []
        self.server_info['CacheMiss'] = []
