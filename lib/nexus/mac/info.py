...
class MacInfo():
    ...
    def get_macs_info(self, cache_enabled=True):
        if cache_enabled:
            if self.mac is not None:
                return self.mac

        managed_objects = self.get_mac_mo(cache_enabled=cache_enabled)
    ...
    def get_macs(self, object_filter=None, cache_enabled=True):
        all_macs = self.get_macs_info(cache_enabled=cache_enabled)
        if all_macs is None:
            self.log.error(
                'get_macs',
