...
class LacpInfo():
    ...
    def get_lacps_info(self, cache_enabled=True):
        if cache_enabled:
            if self.lacp is not None:
                return self.lacp

        managed_objects = self.get_lacp_mo(cache_enabled=cache_enabled)
    ...
    def get_lacps(self, object_filter=None, cache_enabled=True):
        all_lacps = self.get_lacps_info(cache_enabled=cache_enabled)
