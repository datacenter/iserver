...
class LldpInfo():
    ...
    def get_lldps_info(self, cache_enabled=True):
        if cache_enabled:
            if self.lldp is not None:
                return self.lldp

        managed_objects = self.get_lldp_mo(cache_enabled=cache_enabled)
    ...
    def get_lldps(self, object_filter=None, cache_enabled=True):
        all_lldps = self.get_lldps_info(cache_enabled=cache_enabled)
