...
class InputValidator():
    ...
    def get_template_dir(self, template_name):
        ...
        if not os.path.isdir(template_dir):
            self.my_output.error('Template definition not found: %s' % (template_dir))
            return None

        return template_dir

    def get_template_files_dir(self, template_name):
        ...
        if not os.path.isdir(files_dir):
            self.my_output.error('Template files not found: %s' % (files_dir))
            return None

        return files_dir

    def get_template_rules_dir(self, template_name):
        template_dir = self.get_template_dir(template_name)
        if template_dir is None:
            return None
        ...
        if not os.path.isdir(rules_dir):
            self.my_output.error('Template rules not found: %s' % (rules_dir))
            return None

        return rules_dir

    def get_section_rules(self, template_name, section_name):
        rules_dir = self.get_template_rules_dir(template_name)
    ...
    def get_template_name(self, user_input, template_reference=None, template_name=None):
        if template_reference is None and template_name is None:
            self.my_output.error('Define template reference or name')
            return None
        ...
        try:
            ...
            self.my_output.error('Template name expected at %s' % (template_reference))
            return None

        return template_name

    def get_template_definition_key_value(self, template_name, key):
        template_definition = self.get_template_definition(template_name)
        if template_definition is None:
            return None

        if key not in template_definition['main']:
            self.my_output.error('No key %s found in template definition %s' % (key, template_name))
            return None

        return template_definition['main'][key]

    def get_template_definition(self, template_name):
        rules_dir = self.get_template_rules_dir(template_name)
        if rules_dir is None:
            return None
        ...
        if rules['check'] is not None:
            ...
            if rules['type'] == 'str':
                ...
                if rules['check'] == 'local-file':
                    ...
                    if not os.path.isfile(filename):
                        self.my_output.error('Expected attribute %s value is existing local file: %s' % (attribute_name, filename))
                        return False, None
                ...
                if rules['check'] == 'must-be-null':
                    ...
                    self.my_output.error('Expected attribute %s value is null' % (attribute_name))
                    return False, None
        ...
        if section_rules['value'] == 'list_of_dict':
            ...
            if 'min_length' in section_rules:
                if len(user_input) < section_rules['min_length']:
                    self.my_output.error('Section %s is expected to be list with minimum %s elements: %s' % (section_name, section_rules['min_length'], len(user_input)))
                    return None

            if 'max_length' in section_rules:
                if len(user_input) > section_rules['max_length']:
                    self.my_output.error('Section %s is expected to be list with maximum %s elements: %s' % (section_name, section_rules['max_length'], len(user_input)))
                    return None
            ...
            if section_rules['validate']:
                ...
                for value in user_input:
                    ...
                    if section_rules['check'] is not None:
                        ...
                        self.my_output.error('Unsupported check: %s' % (section_rules['check']))
                        return None

        return validated_input

    def validate(self, user_input_location, operation, template_reference=None, template_name=None):
        ...
        for section_name in user_input:
            if section_name in mandatory_sections or section_name in optional_sections:
                section_rules = self.get_section_rules(template_name, section_name)
