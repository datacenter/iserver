...
class K8sNodeNetworkConfigurationPolicyInput():
    def __init__(self):
        pass

    def get_nncp_input_data_lb(self, params, my_output):
    ...
    def get_nncp_input_data_eth(self, params, my_output):
        params['name'] = my_output.get_input('\nInterface name: ')
        if len(params['name']) == 0:
            return None
    ...
    def get_nncp_input_data_bond(self, params, my_output):
        params['name'] = my_output.get_input('\nInterface name: ')
        if len(params['name']) == 0:
            return None
    ...
    def get_nncp_input_data_vlan(self, params, my_output):
    ...
    def get_nncp_input_data_node(self, params, my_output):
        supported_nodes = self.get_nodes_name()
        supported_nodes.append('_workers_')
        supported_nodes.append('_all_')
    ...
    def get_nncp_input_data(self, my_output):
        params = {}
        ...
        if params['type'] == 'lb':
            params = self.get_nncp_input_data_lb(
                params, 
                my_output
            )

        if params['type'] == 'eth':
            params = self.get_nncp_input_data_eth(
                params, 
                my_output
            )

        if params['type'] == 'vlan':
            params = self.get_nncp_input_data_vlan(
                params, 
                my_output
            )

        if params['type'] == 'bond':
            params = self.get_nncp_input_data_bond(
                params, 
                my_output
            )

        params = self.get_nncp_input_data_node(
            params, 
            my_output
        )
        if params is None:
