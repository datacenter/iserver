...
class K8sNodeNetworkConfigurationPolicyGenerate():
    def __init__(self):
        pass
    ...
    def generate_nncp_vlan(self, data):
        interface_mo = {}
    ...
    def generate_nncp_eth(self, data):
        interface_mo = {}
    ...
    def generate_nncp_bond(self, data):
        interface_mo = {}
        interface_mo['name'] = data['name']
    ...
    def generate_nncp_lb(self, data):
        interface_mo = {}
        interface_mo['name'] = data['name']
    ...
    def generate_nncp(self, input_data, my_output):
        data = self.check_nncp_input_data(input_data, my_output)
        if data is None:
            return None
        ...
        for item in data:
            ...
            if self.is_node_network_configuration_policy(policy_name, cache_enabled=False):
                ...
                for interface in item['interfaces']:
                    if interface['type'] == 'vlan':
                        nncp_mo['spec']['desiredState']['interfaces'].append(
                            self.generate_nncp_vlan(interface)
                        )

                    if interface['type'] == 'eth':
                        nncp_mo['spec']['desiredState']['interfaces'].append(
                            self.generate_nncp_eth(interface)
                        )

                    if interface['type'] == 'bond':
                        nncp_mo['spec']['desiredState']['interfaces'].append(
                            self.generate_nncp_bond(interface)
                        )

                    if interface['type'] == 'lb':
                        nncp_mo['spec']['desiredState']['interfaces'].append(
                            self.generate_nncp_lb(interface)
                        )
