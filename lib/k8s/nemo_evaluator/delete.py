class K8sNemoEvaluatorDelete():
    def __init__(self):
        self.nemo_evaluator = None

    def delete_nemo_evaluators(self, my_output=None, wait=True):
        if my_output is not None:
            my_output.default('Delete nim build', before_newline=True, underline=True)

        servings = self.get_nemo_evaluators(
            cache_enabled=False
        )
        if servings is None:
            if my_output is not None:
        ...
        for serving in servings:
            ...
            success = self.delete_nemo_evaluator_mo(
                serving['namespace'],
                serving['name']
            )
            if not success:
            ...
            if wait:
                ...
                if not self.wait_no_nemo_evaluator(serving['namespace'], serving['name']):
                    if my_output is not None:
