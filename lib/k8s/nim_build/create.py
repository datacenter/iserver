...
class K8sNimBuildCreate():
    def __init__(self):
        pass

    def create_nim_build(self, body, my_output=None, confirmation=False, wait=True):
        ...
        if self.is_nim_build(namespace, name, cache_enabled=False):
            if my_output is not None:
        ...
        if not self.create_nim_build_mo(body):
            if my_output is not None:
        ...
        if not self.wait_nim_build(namespace, name, max_time=30):
            if my_output is not None:
