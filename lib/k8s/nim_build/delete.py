class K8sNimBuildDelete():
    def __init__(self):
        self.nim_build = None

    def delete_nim_builds(self, my_output=None, wait=True):
        if my_output is not None:
            my_output.default('Delete nim build', before_newline=True, underline=True)

        servings = self.get_nim_builds(
            cache_enabled=False
        )
        if servings is None:
            if my_output is not None:
        ...
        for serving in servings:
            ...
            success = self.delete_nim_build_mo(
                serving['namespace'],
                serving['name']
            )
            if not success:
            ...
            if wait:
                ...
                if not self.wait_no_nim_build(serving['namespace'], serving['name']):
                    if my_output is not None:
