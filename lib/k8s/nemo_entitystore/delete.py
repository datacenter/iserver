class K8sNemoEntitystoreDelete():
    def __init__(self):
        self.nemo_entitystore = None

    def delete_nemo_entitystores(self, my_output=None, wait=True):
        if my_output is not None:
            my_output.default('Delete nim build', before_newline=True, underline=True)

        servings = self.get_nemo_entitystores(
            cache_enabled=False
        )
        if servings is None:
            if my_output is not None:
        ...
        for serving in servings:
            ...
            success = self.delete_nemo_entitystore_mo(
                serving['namespace'],
                serving['name']
            )
            if not success:
            ...
            if wait:
                ...
                if not self.wait_no_nemo_entitystore(serving['namespace'], serving['name']):
                    if my_output is not None:
