...
class K8sNetworkAttachmentDefinitionVlan():
    def __init__(self):
        pass

    def get_nad_vlan_body(self, namespace, name, master, vlan_id, ipam, address, gateway):
        body = {}
    ...
    def create_nad_vlan(
        ...
        if my_output is not None:
            my_output.default('Create VLAN NAD', before_newline=True, underline=True)
            my_output.default('- namespace: %s' % (namespace))
            my_output.default('- name: %s' % (name))
            my_output.default('- master: %s' % (master))
            my_output.default('- vlan: %s' % (vlan_id))
