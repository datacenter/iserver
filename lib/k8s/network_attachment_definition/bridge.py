...
class K8sNetworkAttachmentDefinitionBridge():
    def __init__(self):
        pass

    def get_nad_bridge_body(self, namespace, name, bridge, ipam, address, gateway, route):
        body = {}
        ...
        if len(route) > 0:
            ...
            for item in route:
                entry = {}
                entry['dst'] = item
                entry['gw'] = gateway.split('/')[0]
                config['ipam']['routes'].append(
                    entry
                )
