...
class K8sStorageClusterInfo():
    def __init__(self):
        self.storage_cluster = None

    def get_storage_cluster_info(self, storage_cluster_mo):
        if storage_cluster_mo is None:
            return None
        ...
        info['spec'] = self.get(storage_cluster_mo, 'spec')
        info['status'] = self.get(storage_cluster_mo, 'status')

        info['local_storage_sc'] = None
        info['storage_class'] = None
        devices_mo = self.get(storage_cluster_mo, 'spec:storageDeviceSets', on_error=[], on_none=[])
        ...
        info['phase'] = self.get(storage_cluster_mo, 'status:phase')
        ...
        info['hostnames'] = self.get(storage_cluster_mo, 'status:nodeTopologies:labels:kubernetes.io/hostname', on_error=[], on_none=[])
        if len(info['hostnames']) == 0:
            info['hostnamesT'] = '--'
        else:
            info['hostnamesT'] = ', '.join(info['hostnames'])
        
        info['version'] = self.get(storage_cluster_mo, 'status:version')
        info['current_mon_count'] = self.get(storage_cluster_mo, 'status:currentMonCount')
        info['expected_osd_count'] = None
        device_sets_mo = self.get(storage_cluster_mo, 'spec:storageDeviceSets')
    ...
    def get_storage_clusters_info(self, cache_enabled=True):
        if cache_enabled:
            if self.storage_cluster is not None:
                return self.storage_cluster

        managed_objects = self.get_storage_cluster_mo(cache_enabled=cache_enabled)
        ...
        for managed_object in managed_objects:
            ...
            storage_cluster_info['info'] = self.get_storage_cluster_info(
                managed_object
            )
            storage_cluster_info['mo'] = managed_object
            self.storage_cluster.append(
                storage_cluster_info
            )

        return self.storage_cluster

    def match_storage_cluster(self, storage_cluster_info, object_filter):
        ...
        for rule in object_filter:
            ...
            if key == 'namespace':
                key_found = True
                if not filter_helper.match_string(value, storage_cluster_info['namespace']):
                    return False

            if key == 'name':
                key_found = True
                if not filter_helper.match_string(value, storage_cluster_info['name']):
                    return False

            if not key_found:
                self.log.error(
                    'match_storage_cluster',
                    'Unsupported key: %s' % (key)
                )

        return True

    def get_storage_clusters(self, object_filter=None, return_mo=False, cache_enabled=True):
        all_storage_clusters = self.get_storage_clusters_info(cache_enabled=cache_enabled)
        if all_storage_clusters is None:
            return None

        storage_clusters = []

        for storage_cluster_info in all_storage_clusters:
            if not self.match_storage_cluster(storage_cluster_info['info'], object_filter):
                continue

            if return_mo:
                storage_clusters.append(
                    storage_cluster_info['mo']
                )
                continue

            storage_clusters.append(
                storage_cluster_info['info']
            )

        return storage_clusters

    def is_storage_cluster(self, cache_enabled=True):
        if self.get_storage_cluster(cache_enabled=cache_enabled) is None:
            return False
        return True

    def get_storage_cluster(self, return_mo=False, cache_enabled=True):
        storage_clusters = self.get_storage_clusters(
            return_mo=return_mo,
            cache_enabled=cache_enabled
        )
        if storage_clusters is None:
            return None

        if len(storage_clusters) == 1:
            return storage_clusters[0]

        return None

    def get_storage_cluster_body(
            self, 
            namespace,
            name,
            storage_class,
    ...
    def create_storage_cluster_from_params(
            self, 
            namespace,
            name,
            storage_class,
        ...
        if self.is_storage_cluster(cache_enabled=False):
            if my_output is not None:
                my_output.default('- storage cluster already defined')
            return True
        
        body = self.get_storage_cluster_body(
            namespace,
            name,
            storage_class,
            local_storage_sc,
        ...
        if not self.create_storage_cluster_mo(body):
            if my_output is not None:
        ...
        if wait:
            ...
            success = self.wait_storage_cluster()
            if not success:
                if my_output is not None:
            ...
            success = self.wait_storage_cluster_ready()
            if not success:
                if my_output is not None:
                    my_output.error('Timed out')
                return False
    ...
    def create_storage_cluster_from_body(
            self, 
            namespace,
            name,
            body,
    ...
    def wait_storage_cluster(self, max_time=360):
        start_time = int(time.time())
        while True:
            storage_cluster_info = self.get_storage_cluster(
                cache_enabled=False
            )
            if storage_cluster_info is not None:
                return True

            duration = int(time.time()) - start_time
            if duration > max_time:
                self.log.error(
                    'k8s.wait_storage_cluster',
                    'Max time reached'
                )
                return False
    ...
    def wait_storage_cluster_ready(self, max_time=360):
        start_time = int(time.time())
        while True:
            storage_cluster_info = self.get_storage_cluster(
                cache_enabled=False
            ...
            if storage_cluster_info is not None:
                if storage_cluster_info['ready']:
                    return True

            duration = int(time.time()) - start_time
            if duration > max_time:
                self.log.error(
                    'k8s.wait_storage_cluster_ready',
                    'Max time reached'
                )
                return False
    ...
    def get_odf_cluster_resource_names(self, my_output=None):
        ...
        storage_cluster = self.get_storage_cluster(cache_enabled=False)
        if storage_cluster is None:
            if my_output is not None:
                my_output.error('Storage cluster object not found')
            return None

        if storage_cluster['expected_osd_count'] is None:
            if my_output is not None:
        ...
        for index in range(0, storage_cluster['expected_osd_count']):
            deployments.append(
                dict(
                    namespace='openshift-storage',
    ...
    def delete_storage_cluster(self, my_output=None, wait=True):
        if my_output is not None:
            my_output.default('Delete storage cluster', before_newline=True, underline=True)

        info = self.get_storage_cluster(cache_enabled=False)
        if info is None:
            my_output.default('- already deleted')
            return True
        
        if not self.delete_storage_cluster_mo(info['namespace'], info['name']):
            if my_output is not None:
                my_output.error('REST API failed')
            return False
        ...
        if wait:
            ...
            if not self.wait_no_storage_cluster():
                if my_output is not None:
                    my_output.error('Timed out')
                return False
    ...
    def wait_no_storage_cluster(self, max_time=60):
        start_time = int(time.time())
        while True:
            storage_cluster_info = self.get_storage_cluster(
                cache_enabled=False
            )
            if storage_cluster_info is None:
                return True
