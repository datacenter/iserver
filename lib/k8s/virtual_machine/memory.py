...
class K8sVirtualMachineMemory():
    ...
    def get_virtual_machine_memory_change_body(self, namespace, name, size):
        ...
        body['spec']['template']['spec']['domain']['memory'] = {}
        body['spec']['template']['spec']['domain']['memory']['guest'] = size
        return body
    
    def change_virtual_machine_memory(self, namespace, name, size, confirmation=False, my_output=None, wait=True, restart=False):
        if my_output is None:
            confirmation = False

        if my_output is not None:
            my_output.default('Virtual Machine Memory Change', before_newline=True, underline=True)
        ...
        if my_output is not None:
            my_output.default('- current memory: %s' % (info['memory']))
            my_output.default('- requested memory: %s' % (size))
        ...
        body = self.get_virtual_machine_memory_change_body(namespace, name, size)
        if my_output is not None:
