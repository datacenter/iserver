...
class K8sVirtualMachineStop():
    ...
    def stop_virtual_machine(self, namespace, name, confirmation=False, my_output=None, wait=True):
        ...
        if my_output is not None:
            my_output.default('Stop Virtual Machine', before_newline=True, underline=True)
        ...
        if vmi is None:
            if my_output is not None:
                my_output.default('- vmi not found <=> vm already stopped')
            return True
        ...
        if info['running'] is not None:
            body = self.get_stop_body_running(namespace, name)
            if my_output is not None:
        ...
        if info['run_strategy'] is not None:
            body = self.get_stop_body_run_strategy(namespace, name)
            if my_output is not None:
                my_output.default(yaml.dump(body), before_newline=True, wrap='~~~')
        ...
        if my_output is not None:
            my_output.default('Wait for virtual machine down')

        success = self.wait_virtual_machine_down(namespace, name)
        if not success:
            if my_output is not None:
        ...
        if my_output is not None:
            my_output.default('Wait for no virtual machine instance')

        success = self.wait_no_virtual_machine_instance(namespace, name)
        if not success:
            if my_output is not None:
                my_output.error('Timed out')
            return False        
